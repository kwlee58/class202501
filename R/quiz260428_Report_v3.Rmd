...
class_roll0504 <- 
  readRDS("./class_roll_260504.RDS")
class_roll0504 %<>% 
  data.frame
class_roll <- 
  read_excel("./data/class_roll_260511.xlsx", 
             range = "B1:H604")
class_roll %<>% 
  data.frame
str(class_roll)
names(class_roll) <- c("dept", "colleges", "id", "name", "status", "email", "cell_no")
class_roll$group <- class_roll0504$group[match(class_roll$id, class_roll0504$id)]
saveRDS(class_roll, file = "./class_roll_260511.RDS")
...
quiz_wrong_ids <- quiz_wrong_ids %>%
  mutate(
    id = case_when(
      id == "020823" ~ "20217089",
      id == "200235173" ~ "20235173",
      id == "202122570" ~ "20212570",
    TRUE ~ id
    ),
    name = case_when(
...
#   quiz[., c("id", "name")]
quiz %>%
  filter(id %in% c("20256639", "20266131"))

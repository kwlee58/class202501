---
title: "Quiz 260428 (Default Effects)"
author: "coop711"
date : "2026-04-28"
...
class_roll %<>% 
  data.frame
# class_roll <- 
#   read_excel("./data/class_roll_251027.xlsx", 
#              range = "B1:H599")
# class_roll %<>% 
#   data.frame
str(class_roll)
# names(class_roll) <- c("dept", "colleges", "id", "name", "status", "email", "cell_no")
# class_roll$group <- class_roll0421$group[match(class_roll$id, class_roll0421$id)]
# saveRDS(class_roll, file = "./class_roll_251027.RDS")
#> googlesheets4 패키지 시험
quiz_url <-"https://docs.google.com/spreadsheets/d/1ldbbtnaAjtYPrvhQTVLWrYIj_rcU7i1VBo1pfPDPPgI"
# gs4_deauth()
quiz <- read_sheet(quiz_url)
# quiz <- 
#   read_excel("./data/quiz251027.xlsx",
#              range = "A2:N720",
#              col_names = FALSE)
str(quiz)
names(quiz) <- 
...
# quiz[quiz$id == "20253225", "time"] <- c("2025-10-27 00:00:01")
# quiz[quiz$id == "20255122", "time"] <- c("2025-10-27 00:00:02")
# quiz[quiz$id == "20225238", "time"] <- c("2025-10-27 00:00:03")
str(quiz)
as.Date(quiz$time) %>%
  table
quiz <- data.frame(quiz)
#> 마감기한으로부터 제출시점의 기간
quiz$days <- as.numeric(difftime("2026-05-12 00:00:00", quiz$time, tz = "Asia/Seoul",  units = 'days'))
quiz$hours <- as.numeric(difftime("2026-05-12 00:00:00", quiz$time, tz = "Asia/Seoul", units = 'hours'))
quiz %>% 
  str  
```
...
quiz_wrong_ids <- quiz_wrong_ids %>%
  mutate(
    id = case_when(
      id == "20243920" ~ "20253920",
      id == "20050616" ~ "20246298",
    TRUE ~ id
    ),
    name = case_when(
      id == "20252639" & name == "mymymy4696*" ~ "홍시원",
      id == "20256726" & name == "박 윤" ~ "박윤",
      id == "20218008" & name == "biexi" ~ "bie xi",
      id == "20178019" & name == "cHENSHUAIHAO" ~ "CHEN SHUAIHAO",
...
#   quiz[., c("id", "name")]
quiz %>%
  filter(id %in% c("20253133", "20246941", "20241232", "20242581", "20243022", "20213020"))
...
m + geom_text(aes(x = center, y = 1.05), 
  ...
  theme(plot.title = element_text(hjust = 0.5, 
                                  size = 18, 
                                  face = "bold")) 
ggsave("./pics/Quiz260428_Q7_RnB.png", dpi = 120, width = 960, height = 840, units = "px")
```
...
title(main = paste0("Quiz260428 (", N, "명 제출)"))
# title(main = "일 단위 퀴즈 제출 인원", sub = paste0("Quiz230925 (", N, "명 제출)"))
dev.copy(png, file = "./pics/2601/quiz_submission_9.png", width = 640, height = 480)
invisible(dev.off())
})
```
...
ggsave("./pics/Quiz260428_submission_9.png", dpi = 120, width = 960, height = 840, units = "px")
```

```{r, save, echo = FALSE}
saveRDS(quiz, file = "quiz260428.RDS")
```

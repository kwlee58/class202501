---
title: "Quiz 250414 (Wason Selection Task)"
author: "coop711"
date: "2025-04-14"
...
정답인 {A,3}를 올린 `r format(tbl_q9_a[1, 2], digits = 3, nsmall = 0)`(명)보다 아주 조금 많습니다.
...
이는 확증편향에서 비롯된 응답이 정답보다 훨씬 많다는 것을 보여줍니다.
...
확증편향으로 인한 응답이 정답보다 훨씬 많다는 것을 다시 확인할 수 있습니다.
...
구체적 질문을 먼저 학습하고 추상적 질문을 학습하는 것과 추상적 질문을 먼저 학습하고 구체적 질문을 학습하는 방식 중에 어느 것이 더 나은지 비교한 결과 순서에 따른 정답인원의 차이에는 통계적으로 유의한 차이가 관찰되었습니다.

어떻게 해석할 수 있을까요? 

추상적 문제를 먼저 학습하는 것이 효과적일까요?

이 문제에 대해서는 학기 마다 조금씩 다른 결과가 나옵니다.
...
그러나, 추상적 질문을 Q8에 배치한 Black 의 경우 `r wason_tbl_a[2, 1]`(명) 이 정답을 올렸고, 구체적 질문에는 171명이 정답을 올려서 통계적으로 유의한 차이를 관찰하였습니다.

카이제곱 통계량은 `r format(chisq.test(wason_tbl_a[, -3])[1], digits = 2, nsmall = 2)`, p-value 는 `r format(chisq.test(wason_tbl_a[, -3])[3], digits = 2, nsmall = 2)`으로 통계적으로 유의한 차이를 관찰하였습니다.

따라서 학습 순서는 추상적 질문의 정답율에 영향을 미치고 있습니다.
...
추상적 질문에 대한 전체 정답 중에서 추상적 질문을 먼저 제시한 Red 가 `r format(wason_ptbl[1, 1], digits = 2, nsmall = 1)`(%)를 차지하여 추상적 질문을 나중에 제시한 Black 보다 높고, 그 차이는 앞에서 살펴 본 것처럼 통계적으로 유의합니다.
...
Mosaic Plot으로부터 구체적 질문이 먼저 주어진 Red나 구체적 질문이 나중에 주어진 Black에서 정답을 올린 인원이나 백분율에 차이가 있다는 것을 시각적으로 파악할 수 있습니다.

### 합산

실험에 참여한 어느 누구나 추상적 문제와 구체적 문제를 한 번씩 풀게 됩니다.

학습 순서의 영향이 있는 것으로 파악되었지만 추상적 문제의 정답률과 구체적 문제의 정답률을 합쳐서 비교해 보겠습니다.

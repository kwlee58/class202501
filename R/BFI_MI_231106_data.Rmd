...
options(width = 180)
class_roll <- 
  read_excel("./data/class_roll231016.xlsx",
             range = "E1:F913",
             col_names = TRUE,
             col_types = "text")
class_roll %<>%
...
```{r}
BFI <- "./data/BFI231106_v2.xlsx" %>% 
  read_excel(range = "A1:AA791",
             col_names = TRUE)
BFI %<>%
  data.frame
BFI %<>%
...
#> 마감일로부터 제출시점의 기간
BFI_days <- as.numeric(difftime("2023-11-20 09:00:00", BFI$Time, 
                                 tz = "Asia/Seoul", units = 'days'))
BFI_hours <- as.numeric(difftime("2023-11-20 09:00:00", BFI$Time, 
...
```{r}
MI <- "./data/MI231106.xlsx" %>% 
  read_excel(range = "A1:BQ822",
             col_names = TRUE)
MI %<>%
  data.frame
MI <- MI[, -2]
...
#> 마감일로부터 제출시점의 기간
MI_days <- 
  as.numeric(difftime("2023-11-20 09:00:00", MI$Time, 
                      tz = "Asia/Seoul", units = 'days'))
MI_hours <- 
  as.numeric(difftime("2023-11-20 09:00:00", MI$Time, 
                      tz = "Asia/Seoul", units = 'hours'))
...
```{r, save.image}
saveRDS(BFI, file = "./BFI231106.RDS")
saveRDS(MI, file = "./MI231106.RDS")
save.image("./BFI_MI_231106.RData")
```

---
title: "Quiz 260414 (Wason Selection Task)"
author: "coop711"
date: "2026-04-14"
...
class_roll0413 <- readRDS("./class_roll_260413.RDS")
# class_roll <- 
#   read_excel("./data/class_roll_251026.xlsx", 
#              range = "B1:H607")
# class_roll %<>% 
#   data.frame
# str(class_roll)
# names(class_roll) <- c("dept", "colleges", "id", "name", "status", "email", "cell_no")
# class_roll$group <- class_roll1005$group[match(class_roll$id, class_roll1005$id)]
# saveRDS(class_roll, file = "./class_roll_251026.RDS")
class_roll <- class_roll0413
#> googlesheets4 패키지 시험
quiz_url <-"https://https://docs.google.com/spreadsheets/d/1FWJIOgyw4auTCcxQmMt3M_beB8U4GEsUmbugkCmjQmI"
# gs4_deauth()
quiz <- read_sheet(quiz_url)
# quiz <- gsheet2tbl(quiz_url)
#> 잘 안됨.  
# quiz <- 
#   read_excel("./data/quiz_251013.xlsx",
#              range = "A2:P746",
#              col_names = FALSE)
names(quiz) <- 
...
fix_wason_response_simple <- function(x) {
  ...
  x <- str_remove_all(x, "[()]")         # (추가) 괄호 제거 (A,2) → A,2
  x <- str_replace_all(x, "，", ",")    # 한글 쉼표 → 일반 쉼표
  ...
  x <- str_replace(x, "^Bear,([0-9]+)$", "Beer,\\1")
  x <- str_replace(x, "^Bee,([0-9]+)$",  "Beer,\\1")   # ✅ 추가: Bee,17 → Beer,17
  x <- str_replace(x, "^Be{2,}r,([0-9]+)$", "Beer,\\1")  # 추가: Beeer → Beer
  x <- str_replace(x, regex("^B[eE]{2,}r,([0-9]+)$", ignore_case = TRUE), "Beer,\\1") 
  x <- str_replace(x, "^([A-Za-z]+)\\.([0-9]+)$", "\\1,\\2")   # 추가: A.2 → A,2
...
quiz[quiz$id == "202522991", "time"] <- c("2025-10-26 23:59:00")
...
#> 마감기한으로부터 제출시점의 기간
quiz$days <- as.numeric(difftime("2026-04-28 00:00:00", quiz$time, units = 'days'))
quiz$hours <- as.numeric(difftime("2026-04-28 00:00:00", quiz$time, units = 'hours'))
str(quiz)
```
...
quiz_wrong_ids <- quiz_wrong_ids %>%
  mutate(
    id = case_when(
      id == "20233105" ~ "20233015",
      id == "202266628" ~ "20226628",
      id == "202522991" ~ "20252991",
    TRUE ~ id
    ),
    name = case_when(
      name == "박 윤" ~ "박윤",
...
#   quiz[., c("id", "name")]
quiz %>%
  filter(id %in% c("20255136"))
# quiz[quiz$name %in% c("김장윤"), ] #> 김장윤이 중복 제출
quiz <- quiz %>% 
...
```{r, Selection Bias, echo = FALSE, warning = FALSE}
tbl_q2_a <- 
  quiz$Q2 %>% 
  table(quiz$group, ., useNA = "ifany") %>%
  cbind("계" = rowSums(.)) %>%
...
m + geom_text(aes(x = center, y = 1.05), 
  ...
  theme(plot.title = element_text(hjust = 0.5, 
                                  size = 18, 
                                  face = "bold")) 
ggsave("./pics/Quiz251013_Q7_RnB.png", dpi = 120, width = 960, height = 840, units = "px")
```
...
m + geom_text(aes(x = center, y = 1.05), 
  ...
  theme(plot.title = element_text(hjust = 0.5, 
                                  size = 18, 
                                  face = "bold")) 
ggsave("./pics/Quiz251013_Q8_RnB.png", dpi = 120, width = 960, height = 840, units = "px")
```
...
정답인 {A,3}를 올린 `r format(tbl_q9_a[1, 2], digits = 3, nsmall = 0)`(명) 다음으로 많습니다.
...
정답인 {A,3}를 올린 `r format(tbl_q9_a[2, 2], digits = 3, nsmall = 0)`(명)보다 조금 많습니다.
...
m + geom_text(aes(x = center, y = 1.05), 
  ...
  theme(plot.title = element_text(hjust = 0.5, 
                                  size = 18, 
                                  face = "bold")) 
ggsave("./pics/Quiz251013_Q9_RnB.png", dpi = 120, width = 960, height = 840, units = "px")
```
...
m + 
  ...
  theme(plot.title = element_text(hjust = 0.5, 
                                  size = 18, 
                                  face = "bold")) 
ggsave("./pics/Quiz251013_Wason_RnB.png", dpi = 120, width = 960, height = 840, units = "px")
```
...
b_fill + 
  ...
  theme(plot.title = element_text(hjust = 0.5, 
                                  size = 18, 
                                  face = "bold")) 
ggsave("./pics/Quiz251013_Wason_Result.png", dpi = 120, width = 960, height = 840, units = "px")
```
...
카이제곱 통계량은 `r format(days_chisq_test[1], digits = 2, nsmall = 2)`, 자유도는 `r format(days_chisq_test[2], digits = 2, nsmall = 2)`, p-value 는 `r format(days_chisq_test[3], digits = 2, nsmall = 4)` 이므로 날짜별로 고르게 제출하지  않았습니다. 
...
title(main = paste0("Quiz260414 (", N, "명 제출)"))
# title(main = "일 단위 퀴즈 제출 인원", sub = paste0("Quiz230925 (", N, "명 제출)"))
dev.copy(png, file = "./pics/2601/quiz_submission_7.png", width = 640, height = 480)
invisible(dev.off())
})
```
...
카이제곱 통계량은 `r format(days_RnB_chisq_test[1], digits = 2, nsmall = 2)`, 자유도는 `r format(days_RnB_chisq_test[2], digits = 2, nsmall = 2)`, p-value 는 `r format(days_RnB_chisq_test[3], digits = 2, nsmall = 4)` 이므로 제출 시간의 분포는 Red, Black 간에 통계적으로 유의한 차이가 관찰되지 않았습니다.
...
ggsave("./pics/Quiz260414_submission_7.png", dpi = 120, width = 960, height = 840, units = "px")
```

```{r, save, echo = FALSE}
saveRDS(quiz, file = "quiz260414.RDS")
```

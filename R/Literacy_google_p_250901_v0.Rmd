...
literacy_roll <- literacy_roll %>%
  select(-dept, -name)
#> literacy_roll 에 group 추가하기 (이번 학기에는 불필요)
# literacy_roll <- literacy_roll %>%
...
```{r, summary by group, echo = FALSE, eval = FALSE}
score %>%
...
```{r, stem for Red, echo = FALSE, eval = FALSE}
score %>%
  `[`(literacy$group == "Red") %>%
  stem(scale = 0.5)
```
...
```{r, stem for Black, echo = FALSE, eval = FALSE}
score %>%
...
```{r, boxplots, echo = FALSE, fig.width = 6, fig.height = 6, eval = FALSE}
score %>%
  plot(literacy$group, .)
```
...
```{r, qqplots, echo = FALSE, fig.width = 6, fig.height = 6, eval = FALSE}
...
```{r, ECDF, echo = FALSE, fig.width = 6, fig.height = 6, eval = FALSE}
score %>%
  `[`(Red & !is.na(score)) %>%
  ecdf %>%
...
명백히 순서형 범주인 문해력 등급이 Red, Black 간에 통계적으로 유의한 차이가 있는 지 판단하기 위해서는 일반적인 **카이제곱 테스트** 보다는 Cochran-Armitage test가 적합합니다. Red와 Black에 들어간 등급별 인원수가 얼마나 닮았는지를 살펴보았지만 t-test와 마찬가지로 통계적으로 유의한 차이가 관찰되지 않았습니다. 

```{r, literacy grade RnB, echo = FALSE, eval = FALSE, warning = FALSE}

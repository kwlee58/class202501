---
title: "Quiz 250505 (Prospect Theory)"
author: "coop711"
date : "2025-05-05"
...
quiz_url <-"https://docs.google.com/spreadsheets/d/1P5TY_yBaGGLhqOrBufssS6xxVFD73z2RKLw3rokPeKw"
# gs4_deauth()
quiz <- read_sheet(quiz_url)
# quiz <- 
#   read_excel("./data/quiz250505.xlsx",
#              range = "A2:N721",
#              col_names = FALSE)
str(quiz)
names(quiz) <- 
...
quiz <- data.frame(quiz)
#> 마감기한으로부터 제출시점의 기간
quiz$days <- as.numeric(difftime("2025-05-19 00:00:00", quiz$time, tz = "Asia/Seoul",  units = 'days'))
quiz$hours <- as.numeric(difftime("2025-05-19 00:00:00", quiz$time, tz = "Asia/Seoul", units = 'hours'))
quiz %>% 
  str  
```
...
quiz_wrong_ids <- quiz_wrong_ids %>%
  mutate(
    id = case_when(
      id == "20221223" ~ "20212223",
      id == "29242345" ~ "20242345",
      id == "20241505" ~ "20241506",
      id == "2019716" ~ "20193716",
      id == "20272744" ~ "20242744",
...
#> 학과 이름에 학번 쓴 사람 처리
quiz[quiz$dept == "20213353", "dept"] <- "디스플레이전공"
quiz[quiz$dept == "20226424", "dept"] <- "글로벌비즈니스전공"
quiz[quiz$dept == "20246627", "dept"] <- "디지털인문예술전공"
#> 중복 제출자
quiz %>%
...
기대값을 계산해 보면 어느 게임을 선택하더라도 상관이 없어야 하는 데 이득 게임인 Red에서는 조금 덜 받더라도이득을 확고히 하려는 게임(A)에, 손실 게임인 Black에서는 손실을 확률적으로 줄이려는 게임(B)을 많이 선택하여 Kahneman과 Tversky의 Prospect Theory 에 부합하는 결과를 보여줍니다.
...
ggsave("./pics/Quiz250505_Q7_RnB.png", dpi = 120, width = 960, height = 840, units = "px")
```
...
title(main = paste0("Quiz250505 (", N, "명 제출)"))
# title(main = "일 단위 퀴즈 제출 인원", sub = paste0("Quiz230925 (", N, "명 제출)"))
dev.copy(png, file = "./pics/2501/quiz_submission_10.png", width = 640, height = 480)
invisible(dev.off())
})
```
...
ggsave("./pics/Quiz250505_submission_10.png", dpi = 120, width = 960, height = 840, units = "px")
```

```{r, save, echo = FALSE}
saveRDS(quiz, file = "quiz250505.RDS")
```

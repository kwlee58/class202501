---
title: "Quiz 260421 (Cognitive Reponse Test)"
author: "coop711"
date : "2026-04-21"
...
class_roll0427 <- 
  readRDS("./class_roll_260427.RDS")
class_roll0427 %<>% 
  data.frame
class_roll <- 
  read_excel("./data/class_roll_260504.xlsx", 
             range = "B1:H605")
class_roll %<>% 
  data.frame
str(class_roll)
names(class_roll) <- c("dept", "college", "id", "name", "status", "email", "cell_no")
class_roll$group <- class_roll0427$group[match(class_roll$id, class_roll0427$id)]
saveRDS(class_roll, file = "./class_roll_260504.RDS")
#> googlesheets4 패키지 시험
quiz_url <-"https://docs.google.com/spreadsheets/d/1SEPD03OqwECJu4D1_ebYMN6SmDdrXJnGIUpMK-h0IKA"
# gs4_deauth()
quiz <- read_sheet(quiz_url)
# quiz <- 
#   read_excel("./data/quiz251020.xlsx",
#              range = "A2:O741",
#              col_names = FALSE)
names(quiz) <- 
...
as.Date(quiz$time) %>%
  table
#> 마감기한으로부터 제출시점의 기간
quiz$days <- as.numeric(difftime("2026-05-05 00:00:00", quiz$time, units = 'days'))
quiz$hours <- as.numeric(difftime("2026-05-05 00:00:00", quiz$time, units = 'hours'))
str(quiz)
```
...
quiz_wrong_ids <- quiz_wrong_ids %>%
  mutate(
    id = case_when(
      id == "20236748" ~ "20236738",
      id == "20277147" ~ "20266147",
      id == "2021102" ~ "20211023",
      id == "202666163" ~ "20266163",
      id == "201253814" ~ "20253814",
      id == "202111727" ~ "20211727",
      id == "060531" & dept == "20252362" ~ "20252362",
      id == "20040501" & dept == "20235261" ~ "20235261",
      TRUE ~ id
      ),
    name = case_when(
...
#   quiz[., c("id", "name")]
quiz %>%
  filter(id %in% c("20256639", "20255149"))
...
```{r, NFIP, echo = FALSE, warning = FALSE}
tbl_q3_a <- quiz$Q3 %>%
  table(quiz$group, ., useNA = "ifany") %>%
  cbind("계" = rowSums(.)) %>%
  rbind("계" = colSums(.))
...
m4 + geom_text(aes(x = center, y = 1.05), 
  ...
  theme(plot.title = element_text(hjust = 0.5, 
                                  size = 18, 
                                  face = "bold")) 
ggsave("./pics/Quiz260421_Q4_RnB.png", dpi = 120, width = 960, height = 840, units = "px")
```
...
m5 + geom_text(aes(x = center, y = 1.05), 
  ...
  theme(plot.title = element_text(hjust = 0.5, 
                                  size = 18, 
                                  face = "bold")) 
ggsave("./pics/Quiz260421_Q5_RnB.png", dpi = 120, width = 960, height = 840, units = "px")
```
...
m6 + geom_text(aes(x = center, y = 1.05), 
  ...
  theme(plot.title = element_text(hjust = 0.5, 
                                  size = 18, 
                                  face = "bold")) 
ggsave("./pics/Quiz260421_Q6_RnB.png", dpi = 120, width = 960, height = 840, units = "px")
```
...
title(main = paste0("Quiz260421 (", N, "명 제출)"))
# title(main = "일 단위 퀴즈 제출 인원", sub = paste0("Quiz230925 (", N, "명 제출)"))
dev.copy(png, file = "./pics/2601/quiz_submission_8.png", width = 640, height = 480)
invisible(dev.off())
})
```
...
ggsave("./pics/Quiz260421_submission_8.png", dpi = 120, width = 960, height = 840, units = "px")
```

```{r, save, echo = FALSE}
saveRDS(quiz[c("id", "name")], file = "./quiz260421.RDS")
```

...
literacy_roll <- 
  read_excel("./data/literacy_230329.xlsx",
             range = "C1:AD920",
             col_names = TRUE)
...
class_roll$id %in% literacy_roll$id %>%
  ...
  which %>%
  `[`(class_roll, ., "id")
## 정하은 양의 missing group = "Red"
# literacy_roll[746, "group"] <- "Red"
...
# literacy_roll[literacy_roll$id %in% setdiff(literacy_roll$id, class_roll$id), c("id", "name", "dept")]
saveRDS(literacy_roll, file = "./literacy_roll230329.RDS")
```
...
# png(file = "../pics/Literacy_230329.png", width = 640, height = 840)
...
# dev.off()
dev.copy(png, file = "./pics/Literacy_230329.png", width = 640, height = 840)
```
...
#> x축의 라벨을 막대에 보다 가깝게 조정 (flip해서 x축을 수직으로 세울 예정이므로 y = -30)
  geom_text(mapping = aes(y = -20, label = Questions)) 
#> 좌표를 90도 회전하여 막대들을 수평으로 눕히기
#  coord_flip()
ggsave(file = "./pics/Literacy_230329_ggplot.png", bg = "white", dpi = 72, width = 840, height = 480, units = "px")
```
...
Red 와 Black으로부터 관찰된 점수들의 평균에 대하여 t test를 적용하였더니 기초통계량에서 살핀 것을 뒷받침해 주듯이 p-value 가 0.767로 매우 큰 값이 나왔습니다. 평균은 매우 닮았다는 뜻입니다.

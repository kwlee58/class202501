---
title: "Quiz 251124 (Birthday Problem, Monty Hall, ..."
author: "coop711"
date : "2025-11-24"
...
quiz_url <-"https://docs.google.com/spreadsheets/d/1BC_P6QvNzNSL9wqscoy7NwxsptCYbIHqfQ7W9uKbIgs"
...
quiz_wrong_ids <- quiz_wrong_ids %>%
  mutate(
    id = case_when(
      ...
      id == "20245735" ~ "20246735",
      id == "202533249" ~ "20253249",
      id == "20353941" ~ "20253941",
      id == "28178019" & name == "CHEN SHUAIHAO" ~ "20178019",
    TRUE ~ id
    ),
    name = case_when(
      id == "20256726" & name == "박 윤" ~ "박윤",
      id == "20218008" & name == "biexi" ~ "bie xi",
      id == "28178019" & name == "CHEN SHUAIHAO" ~ "CHEN SHUAIHAO",
      id == "20238010" & name == "xuchenxing" ~ "xu chenxing",
    TRUE ~ name
    )
  )
...
quiz[quiz$id == "20253042", "time"] <- c("2025-11-24 00:00:01")
## 자정 직후 제출한 사람의 시간 수동 조정
quiz[quiz$id == "20252998", "time"] <- c("2025-12-07 23:59:00")
## 집계시점으로부터 제출시간까지의 소요일수
quiz$days <- 
  as.numeric(difftime("2025-12-08 00:00:00", 
...
M19_2 <- c(62, 84)
MPG <- 
  cbind(M19_2, M20_1, M20_2, M21_1, M21_2, M22_1, M22_2, M23_1, M23_2, M24_1, M24_2, M25_1, M25_2)
semesters <- 
  c("19/2", "", "20/2", "21/1", "21/2", "22/1", "22/2", "23/1", "23/2", "24/1", "24/2", "25/1", "25/2")
...
m + geom_text(aes(x = center, y = 1.05),
  ...
  theme(plot.title = element_text(hjust = 0.5, 
                                  size = 18, 
                                  face = "bold")) 
ggsave("./pics/quiz251124_Q4_RnB.png", dpi = 120, width = 960, height = 840, units = "px")
```
...
M16_1 <- c(41, 15)
quizS <- 
  cbind(M16_1, M16_2, M17_1, M17_2, M18_2, M18_2, M19_1, M19_2, M20_1, M20_2, M21_1, M21_2, M22_1, M22_2, M23_1, M23_2, M24_1, M24_2, M25_1, M25_2)
semesters <- 
  c("16/1", "16/2", "17/1", "17/2", "18/1", "18/2", "19/1", "19/2", "20/1", "20/2", "21/1", "21/2", "22/1", "22/2", "23/1", "23/2", "24/1", "24/2", "25/1", "25/2")
...
y_max <- sort(unique(p_df$y_breaks[p_df$y_breaks < 0.99]), decreasing = TRUE)[1]
custom_labels <-
  c("16/1", "", "", "", "18/1", "", "19/1", "", "20/1", "", "21/1", "21/2", "22/1", "22/2", "23/1", "23/2", "24/1", "24/2", "25/1", "25/2")
m_list$m +
  geom_text(aes(x = center, y = 1.05),
...
title(main = paste0("quiz251124 (", N, "명 제출)"))
# title(main = "일 단위 퀴즈 제출 인원", sub = paste0("quiz230925 (", N, "명 제출)"))
dev.copy(png, file = "./pics/2502/quiz_submission_13.png", width = 640, height = 480)
invisible(dev.off())
})
```
...
ggsave("./pics/quiz251124_submission_13.png", dpi = 120, width = 960, height = 840, units = "px")
```
...
library(magick)
file_names <- paste0("./pics/2502/quiz_submission_", 2:13, ".png")
...
```{r, save, echo = FALSE}
saveRDS(quiz, file = "./quiz251124.RDS")
```
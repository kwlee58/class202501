...
class_roll0921 <- 
  readRDS("./class_roll_250921.RDS")
class_roll0921 <- data.frame(class_roll0921)
## 수강 철회 반영 새 출석부
...
class_roll <- class_roll0928
sheet_url <- "https://docs.google.com/spreadsheets/d/1dw16Vx1cnKrOODjP8Tgbn3KbchTA1M1txETx8FB1VAQ"
quiz <- 
...
#                       units = 'days'))
quiz$days <- 
  as.numeric(difftime("2025-09-29 00:00:00", quiz$time, units = 'days'))
quiz$hours <- 
  as.numeric(difftime("2025-09-29 00:00:00", quiz$time, units = 'hours'))
str(quiz)
# pander(summary(quiz))
```
...
quiz_wrong_ids <- quiz_wrong_ids %>%
  mutate(id = case_when(
    id == "2023016" ~ "20253016",
    id == "20204134" ~ "20204133",
    id == "1" & name == "ㅈ" & dept == "ㅈ" ~ "20255152",
    id == "29256308" ~ "20256308",
    TRUE ~ id
  ))
...
m + geom_text(aes(x = center, y = 1.05), 
  ...
  theme(plot.title = element_text(hjust = 0.5, 
                                  size = 18, 
                                  face = "bold")) 
ggsave("./pics/Quiz250915_Q7_RnB.png", dpi = 120, width = 960, height = 840, units = "px")
```
...
dev.copy(png, file = "./pics/2502/quiz_submission_3.png", width = 640, height = 480)
invisible(dev.off())
})
```
...
ggsave("./pics/Quiz250915_submission_3.png", dpi = 120, width = 960, height = 840, units = "px")
```

```{r, save, echo = FALSE}
saveRDS(quiz, file = "./quiz250915.RDS")
```

...
class_roll0405 <- readRDS("./class_roll_240405.RDS")
class_roll0405 %<>% 
  data.frame
class_roll <- 
  read_excel("./data/class_roll240412.xlsx", 
             range = "B1:I898")
class_roll %<>% 
  data.frame
str(class_roll)
names(class_roll) <- c("college", "dept", "year", "id", "name", "status", "email", "cell_no")
class_roll$group <- class_roll0405$group[match(class_roll$id, class_roll0405$id)]
saveRDS(class_roll, file = "./class_roll_240412.RDS")
quiz <- 
  read_excel("./data/quiz240412.xlsx",
             range = "A2:P757",
             col_names = FALSE)
names(quiz) <- 
...
absentee$group %>%
  table
#> 마감기한으로부터 제출시점의 기간
quiz$days <- as.numeric(difftime("2024-04-26 09:00:00", quiz$time, units = 'days'))
quiz$hours <- as.numeric(difftime("2023-04-26 09:00:00", quiz$time, units = 'hours'))
str(quiz)
```
...
랜덤화출석부에 있는 Red, Black 과 실제 구글예습퀴즈에 올린 Red, Black 이 다른 사람들이 있어서 파악해 보았습니다만, 바꿔 적은 인원들이 4명에 불과합니다. Red를 Black 이라고 한 사람은 없고, Black을 Red 라고 한 사람의 수효가 4명입니다. 
...
```{r, save, echo = FALSE}
saveRDS(class_roll, file = "./class_roll240412.RDS")
saveRDS(quiz, file = "quiz240412.RDS")
save.image("quiz240412.RData")
```

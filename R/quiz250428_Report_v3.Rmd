---
title: "Quiz 250428 (Default Effects)"
author: "coop711"
# date : "2025-04-28"
...
class_roll0421 <- 
  readRDS("./class_roll_250421.RDS")
class_roll0421 %<>% 
  data.frame
class_roll <- 
  read_excel("./data/class_roll_250428.xlsx", 
             range = "B1:H599")
class_roll %<>% 
  data.frame
str(class_roll)
...
saveRDS(class_roll, file = "./class_roll_250428.RDS")
#> googlesheets4 패키지 시험
quiz_url <-"https://docs.google.com/spreadsheets/d/1WYj2iMnxLTqhlgr6yaSK0o1UoZpZsTN9hzVkJDdqF1E"
# gs4_deauth()
quiz <- read_sheet(quiz_url)
# quiz <- 
#   read_excel("./data/quiz250428.xlsx",
#              range = "A2:N720",
#              col_names = FALSE)
str(quiz)
names(quiz) <- 
...
quiz <- data.frame(quiz)
#> 마감기한으로부터 제출시점의 기간
quiz$days <- as.numeric(difftime("2025-05-12 00:00:00", quiz$time, tz = "Asia/Seoul",  units = 'days'))
quiz$hours <- as.numeric(difftime("2025-05-12 00:00:00", quiz$time, tz = "Asia/Seoul", units = 'hours'))
quiz %>% 
  str  
```
...
그 결과 카이제곱 통계량은 `r chisq_test_q5[1] %>% format(digits = 2, nsmall = 3)`, 자유도는 `r chisq_test_q5[2] %>% format(digits = 2, nsmall = 2)`, p-value 는 `r chisq_test_q5[3] %>% format(digits = 2, nsmall = 4)`이므로 Red, Black 간에 통계적으로 유의한 차이를 보이고(지) 있(않)습니다. 
...
ggsave("./pics/Quiz250428_Q7_RnB.png", dpi = 120, width = 960, height = 840, units = "px")
```
...
title(main = paste0("Quiz250428 (", N, "명 제출)"))
...
ggsave("./pics/Quiz250428_submission_9.png", dpi = 120, width = 960, height = 840, units = "px")
```

```{r, save, echo = FALSE}
saveRDS(quiz, file = "quiz250428.RDS")
```

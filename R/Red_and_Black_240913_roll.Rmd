...
class_roll_id_new2 <- readRDS("./class_roll_240913_id_new2.RDS")
...
#  rename("학번" = id, "이름" = name, "그룹" = group) %>%
#  setNames(c("학번", "이름", "그룹")) %>%
  pander(col.names = c("학번", "이름", "그룹"), row.names = FALSE)
```
...
wide_class_roll %>%
  ...
  arrange(name) %>%
  pander(caption = "가나다순", col.names = c("학번", "이름", "8월24일 그룹", "9월6일 그룹"))

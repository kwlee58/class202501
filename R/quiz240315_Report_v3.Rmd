...
class_roll <- readRDS("./class_roll_240314.RDS")
class_roll <- data.frame(class_roll)
quiz <- 
  read_excel("./data/quiz240315.xlsx",
             range = "A2:N790",
             col_names = FALSE)
names(quiz) <- 
...
#> 마감기한으로부터 제출시점의 기간
quiz$days <- 
  as.numeric(difftime("2024-03-29 09:00:00", quiz$time, units = 'days'))
quiz$hours <- 
  as.numeric(difftime("2024-03-29 09:00:00", quiz$time, units = 'hours'))
str(quiz)
```
...
ggsave("./pics/Quiz240315_Q7_RnB.png", dpi = 120, width = 960, height = 840, units = "px")
```
...
title(main = paste0("Quiz240315 (", N, "명 제출)"))
...
m2 + geom_text(aes(x = center, y = 1.05), 
  ...
  theme(plot.title = element_text(hjust = 0.5, 
                                  size = 18, 
                                  family = "KoPubWorldDotum Bold")) 
ggsave("./pics/Quiz240315_submission_3.png", dpi = 120, width = 960, height = 840, units = "px")
```

```{r, save, echo = FALSE}
saveRDS(quiz, file = "./quiz240315.RDS")
save.image("./quiz240315.RData")
```
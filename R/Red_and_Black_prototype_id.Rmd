...
M1   <- 1                              # 시작 seed
M2   <- 1e2                            # 끝 seed. 소규모 시리즈는 1e2 -> 1e3 -> 1e4 -> 1e5 로 네 번,
                                       #          본탐색은 1e7 (100만씩 열 번이면 M1/M2 를 옮겨가며)
...
## 2026-09-04 수정 : format() 이 큰 수에 지수 표기를 써서 1e+06 으로 찍혔다.
##                   scientific = FALSE 를 붙여 1,000,000 으로 나오게 한다.
fmt <- function(x) format(x, big.mark = ",", scientific = FALSE, trim = TRUE)
cat(sprintf("## Red and Black %s : %s (%s ~ %s Trials)\n\n",
            ROLL, VARS_LABEL, fmt(M1), fmt(M2)))
cat(sprintf("*출석부 %s · %d명 · 시행 %s회*\n\n",
            ROLL, nrow(class_roll), fmt(M2 - M1 + 1)))
```

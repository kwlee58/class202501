...
class_roll0329 <- readRDS("./class_roll_240329.RDS")
class_roll0329 %<>% 
  data.frame
class_roll <- 
  read_excel("./data/class_roll240405.xlsx", 
             range = "B1:I900")
class_roll %<>% 
  data.frame
str(class_roll)
names(class_roll) <- c("college", "dept", "year", "id", "name", "status", "email", "cell_no")
class_roll$group <- class_roll0329$group[match(class_roll$id, class_roll0329$id)]
quiz <- 
  read_excel("./data/quiz240405.xlsx",
             range = "A2:N763",
             col_names = FALSE)
str(quiz)
names(quiz) <- c("time", "email", "dept", "id", "name", paste0("Q", 1:6), "group", "Q7_R", "Q7_B")
saveRDS(class_roll, file = "./class_roll_240405.RDS")
...
#> 마감기한으로부터 제출시점의 기간
#> 제출시간 분포 분석 코드 
quiz$days <- as.numeric(difftime("2024-04-19 09:00:00", quiz$time, units = 'days'))
quiz$hours <- as.numeric(difftime("2024-04-19 09:00:00", quiz$time, units = 'hours'))
str(quiz)
```
...
ggsave("./pics/Quiz240405_Q7_RnB.png", dpi = 120, width = 960, height = 840, units = "px")
```
...
title(main = paste0("Quiz240405 (", N, "명 제출)"))
# title(main = "일 단위 퀴즈 제출 인원", sub = paste0("Quiz230925 (", N, "명 제출)"))
dev.copy(png, file = "./pics/2401/quiz_submission_6.png", width = 640, height = 480)
invisible(dev.off())
```
...
m2 + geom_text(aes(x = center, y = 1.05), 
  ...
  theme(plot.title = element_text(hjust = 0.5, 
                                  size = 18, 
                                  family = "KoPubWorldDotum Bold")) 
ggsave("./pics/Quiz240405_submission_5.png", dpi = 120, width = 960, height = 840, units = "px")
```


```{r, save, echo = FALSE}
saveRDS(class_roll, file = "./class_roll240405.RDS")
saveRDS(quiz, file = "./quiz240405.RDS")
save.image("./quiz240405.RData")
```

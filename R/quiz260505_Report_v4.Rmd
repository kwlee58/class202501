---
title: "Quiz 260505 (Prospect Theory)"
author: "coop711"
date : "2026-05-05"
...
class_roll0511 <-
  readRDS("./class_roll_260511.RDS")
class_roll0511 %<>%
  data.frame
class_roll <-
  read_excel("./data/class_roll_260518.xlsx",
             range = "B1:H603")
class_roll %<>%
  data.frame
str(class_roll)
names(class_roll) <- c("dept", "colleges", "id", "name", "status", "email", "cell_no")
class_roll$group <- class_roll0511$group[match(class_roll$id, class_roll0511$id)]
saveRDS(class_roll, file = "./class_roll_260518.RDS")
#> googlesheets4 패키지 시험
quiz_url <-"https://docs.google.com/spreadsheets/d/17akznwm26259qLzt5JvwbrBIL-Hcv7T7dTCtXJAuc0s"
# gs4_deauth()
quiz <- read_sheet(quiz_url)
# quiz <-
#   read_excel("./data/quiz251103.xlsx",
#              range = "A2:N721",
#              col_names = FALSE)
str(quiz)
names(quiz) <-
...
quiz <- data.frame(quiz)
#> 마감기한으로부터 제출시점의 기간
quiz$days <- as.numeric(difftime("2026-05-19 00:00:00", quiz$time, tz = "Asia/Seoul",  units = 'days'))
quiz$hours <- as.numeric(difftime("2026-05-19 00:00:00", quiz$time, tz = "Asia/Seoul", units = 'hours'))
quiz %>%
  str
```

```{r, time filter, echo = FALSE, results = 'asis'}
#> 제출 가능 구간 정의 (한국시간 기준)
#>   - 시작: 2026-05-05 00:00:00 (수업일)
#>   - 마감: 2026-05-19 00:00:00 (다음 수업 직전)
submit_start <- as.POSIXct("2026-05-05 00:00:00", tz = "Asia/Seoul")
submit_end   <- as.POSIXct("2026-05-19 00:00:00", tz = "Asia/Seoul")

#> quiz$time 이 character 로 변환되어 있으므로 POSIXct 로 다시 해석
quiz_time_kst <- as.POSIXct(quiz$time, tz = "Asia/Seoul")

#> 제출 가능 구간을 벗어난 응답 확인 (early: 시작 전, late: 마감 후)
out_of_window <- quiz %>%
  mutate(
    submit_time = quiz_time_kst,
    status = case_when(
      submit_time < submit_start ~ "early",
      submit_time > submit_end   ~ "late",
      TRUE                       ~ "on_time"
    )
  ) %>%
  filter(status != "on_time") %>%
  select(time, id, name, status) %>%
  arrange(time)
out_of_window %>% print
out_of_window %>% count(status) %>% print

#> 구간 안의 응답만 남기기 (조기 제출 + 지각 제출 모두 제거)
quiz <- quiz[!is.na(quiz_time_kst) &
               quiz_time_kst >= submit_start &
               quiz_time_kst <= submit_end, ]
cat("필터 후 남은 응답 수:", nrow(quiz), "\n")
```
...
quiz_wrong_ids <- quiz_wrong_ids %>%
  mutate(
    id = case_when(
      id == "202480" & name == "BAI YUANYUAN" ~ "20248012",
      id == "202526222" ~ "20252622",
      id == "20233206" ~ "20233026",
      id == "20246630" ~ "20246620",
      id == "20056189" ~ "20256189",
    TRUE ~ id
    ),
    name = case_when(
      id == "20258044" & name == "Zhumabaeva Asema" ~ "ZHUMABAEVA ASEMA",
      id == "20248020" & name == "유주" ~ "LIU ZHOU",
      id == "20232569" & name == "040814" ~ "임미영",
      TRUE ~ name
      )
    )
...
#> 학과 이름에 학번 쓴 사람 처리
quiz[quiz$dept == "202255136", "dept"] <- "소프트웨어학부"
#> 중복 제출자
quiz %>%
...
#   quiz[., c("id", "name")]
quiz %>%
  filter(id %in% c("20252203", "20252347"))
quiz <- quiz %>%
...
m + geom_text(aes(x = center, y = 1.05),
  ...
  theme(plot.title = element_text(hjust = 0.5,
                                  size = 18,
                                  face = "bold"))
ggsave("./pics/Quiz251103_Q7_RnB.png", dpi = 120, width = 960, height = 840, units = "px")
```
...
title(main = paste0("Quiz260505 (", N, "명 제출)"))
# title(main = "일 단위 퀴즈 제출 인원", sub = paste0("Quiz230925 (", N, "명 제출)"))
dev.copy(png, file = "./pics/2601/quiz_submission_10.png", width = 640, height = 480)
invisible(dev.off())
})
```
...
ggsave("./pics/Quiz260505_submission_10.png", dpi = 120, width = 960, height = 840, units = "px")
```

```{r, save, echo = FALSE}
saveRDS(quiz, file = "quiz260505.RDS")
```

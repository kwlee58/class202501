...
# class_roll <- readRDS("./class_roll_251220.RDS")   # (미사용) 2601 RDS 없으면 주석 유지
total <-
  read_excel(path = "./data/Final_Records_2601_0620.xlsx",
...
# army_ID: SmartLEAD 참여자목록에서 역할 '군휴학생'으로 지정된 학생 (2026-06-20 기준 18명)
army_ID <-
  c("20245105",  # 강수하
    "20206103",  # 구본열
    "20246104",  # 권우진
...
## FA(미이수): 전자출결 FA관리 확정자 (2026-06-20 기준, 15명)
FA_id <-
  c("20251062", "20251710", "20211723", "20262316", "20212946",
    "20242976", "20253117", "20244107", "20221004", "20235173",
    "20255183", "20243217", "20192508", "20243358", "20236607")
...
Counts_tbl <- total$Total %>%
  cut(breaks = c(0, 20, 40, 60, 91, 97, 100),
      labels = c("[0 ~ 20)",
                 "[20 ~ 40)",
                 "[40 ~ 60)",
                 "[60 ~ 91)",
...
# str(total)
options(digits = 5)
total_disp <- total[order(total$ID), ]
score_cols <- c("Online_Att", "Online_Quiz", "TED_Quiz",
                "Data_Lab", "Google_Att", "Total")
total_disp[score_cols] <- lapply(total_disp[score_cols],

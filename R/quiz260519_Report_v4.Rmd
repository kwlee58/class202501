---
title: "Quiz 260519 (Framing Effects)"
author: "coop711"
date : "2026-05-19"
...
quiz_url <-"https://docs.google.com/spreadsheets/d/1VI8H6CWTqCeyEwV2eINwYxkwvjU3v5uxvPoxHoFh8LM"
# gs4_deauth()
quiz <- read_sheet(quiz_url)
# quiz <- 
#   read_excel("./data/quiz260519.xlsx",
#              range = "A1:Y701")
names(quiz) <- 
...
quiz[quiz$id == "20256131", "time"] <- c("2025-11-17 00:00:01")
quiz[quiz$id == "20253042", "time"] <- c("2025-11-17 00:00:02")
str(quiz)
as.Date(quiz$time) %>%
  table
#> 마감기한으로부터 제출시점의 기간
quiz$days <- 
  as.numeric(difftime("2026-06-02 00:00:00", 
...
quiz_wrong_ids <- quiz_wrong_ids %>%
  mutate(
    id = case_when(
      id == "2024564" ~ "20242564",
      id == "20322236" ~ "20232236",
      id == "2024305" ~ "20243056",
      id == "2024816" ~ "20248016",
      id == "20242204" ~ "20242205",
    TRUE ~ id
  ),
  name = case_when(
...
title(main = paste0("Quiz260519 (", N, "명 제출)"))
# title(main = "일 단위 퀴즈 제출 인원", sub = paste0("Quiz230925 (", N, "명 제출)"))
dev.copy(png, file = "./pics/2601/quiz_submission_12.png", width = 640, height = 480)
invisible(dev.off())
})
```
...
ggsave("./pics/Quiz260519_submission_12.png", dpi = 120, width = 960, height = 840, units = "px")
```

```{r, save, echo = FALSE}
saveRDS(quiz, file = "quiz260519.RDS")
```

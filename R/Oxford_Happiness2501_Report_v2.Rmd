...
이 때 t-통계량은 `r format(Scores_t_test[1], digits = 2, nsmall = 2)`, 자유도는 `r format(Scores_t_test[2], digits = 2, nsmall = 2)`, p-value 는 `r format(Scores_t_test[3], digits = 2, nsmall = 4)`라고 나옵니다. 
...
그 결과 코크란-아미키지 통계량은 `r format(oxford_CA_test[1], digits = 2, nsmall = 2)`,  `dim`은 `r format(oxford_CA_test[2], digits = 2, nsmall = 2)`, p-value 는 `r format(oxford_CA_test$p.value, digits = 2, nsmall = 4)`이므로 통계적으로 유의한 차이를 보이고 있지 않습니다.
...
t-통계량은 `r format(Duration_t_test[1], digits = 2, nsmall = 2)`, 
자유도는 `r format(Duration_t_test[2], digits = 2, nsmall = 2)`, 
p-value 는 `r format(Duration_t_test[3], digits = 2, nsmall = 2)`이어서 
...
```{r, save, echo = FALSE}
saveRDS(oxford[c("id", "name")], file = "./oxford250421.RDS")
```

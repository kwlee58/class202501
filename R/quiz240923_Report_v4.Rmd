...
Q7의 Red는 주당 근로 69시간제의 도입에 찬반을 묻는 질문 중 찬성을 유도하는 부연설명을 붙였을 때  `r tbl_q7_a[1, 4]`명이 응답한 가운데 `r tbl_q7_a[1, 1]`명이 "찬성한다"는 반응을 보이고, `r tbl_q7_a[1, 2]`명이 "반대한다"는 반응을 보입니다. Black은 같은 상황에서 반대를 유도하는 부연설명을 붙였을 떄 `r tbl_q7_a[2, 4]`명이 응답한 가운데 `r tbl_q7_a[2, 1]`명이 "찬성한다"는 반응을 보이고, `r tbl_q7_a[2, 2]`명이 "빈대한다"는 반응을 보입니다. 그리고 "모름/무응답"에 답한 인원은 Red에  `r tbl_q7_a[1, 3]`명, Black 에 `r tbl_q7_a[2, 3]`명이 응답하였습니다. 카이제곱 테스트는 이와 같은 상황에서 
찬성을 유도하는 부연설명을 붙인 경우와 반대를 유도하는 부연설명을 붙인 경우에 그 차이가 통계적으로 매우, 매우, ... 유의하다는 것을 보여 줍니다. 카이제곱 통계량은 `r chisq_test_q7[1] %>% format(digits = 2, nsmall = 3)`, 자유도는 `r chisq_test_q7[2] %>% format(digits = 2, nsmall = 2)`, p-value 는 `r chisq_test_q7[3] %>% format(digits = 2, nsmall = 4)`으로
...
찬성을 유도하는 부연설명을 붙인 Red에서 "찬성한다"고 응답하는사람들의 백분율, `r format(ptbl_q7_a[1, 1], digits = 2, nsmall = 1)`(%)은 "반대한다"고 응답하는 사람들의 백분율, `r format(ptbl_q7_a[1, 2], digits = 2, nsmall = 1)`(%) 보다 높습니다. 반면 반대를 유도하는 부연설명을 붙인 Black에서 "찬성한다"고 응답하는 사람들의 백분율, `r format(ptbl_q7_a[2, 1], digits = 2, nsmall = 1)`(%)은 "반대한다"고 응답하는 사람들의 백분율, `r format(ptbl_q7_a[2, 2], digits = 2, nsmall = 1)`(%) 보다 훨씬 적습니다. 찬성을 유도하는 부연설명을 붙이느냐, 반대를 유도하는 부연설명을 붙이느냐에 따라 반응이 달라진다는 것을 잘 알 수 있습니다. Red 와 Black 이 워낙 차이가 나지만 전체적으로 어느 정도가 "찬성한다"하고 어느 정도가 "반대한다"고 응답하였는지 합쳐 보겠습니다.
...
"찬성한다"고 응답한 백분율은 Red, Black 합쳐서  `r format(ptbl_q7[1], digits = 2, nsmall = 1)`(%)(으)로  '반 대한다"고 응답한 백분율, `r format(ptbl_q7[2], digits = 2, nsmall = 1)`(%) 보다 상당히 적습니다. 다만, 모름/무응답이 `r format(ptbl_q7[3], digits = 2, nsmall = 1)`(%)로 적지 않습니다.
...
Mosaic Plot 은 이 집계결과를 시각적으로 잘 보여줍니다. 찬성을 유도하는 부연설명을 붙인 Red 에서 "찬성한다"고 응답한 백분율이 높고, 반대를 유도하는 부연설명을 붙인 Black 에서 "반대한다"고 응답한 백분율이 월등히 높은 것을 시각적으로 알 수 있습니다. 

# 제출 시간의 분포

과제 제출이 제출 기간 마지막 날에 몰린다는 것을 시각적으로 보여주고 싶어서 하나 추가하였습니다. 주차가 넘어가면서 마감일에 몰리는 현상이 뚜럇해지고 있습니다. 여기서조차 랜덤화 효과를 관찰할 수 있네요. p-value 에 유의해 주세요. 제출시간과 관련한 두 가지 현상에 대해서도 여러분의 생각을 들어보고 싶습니다. 첫째, 랜덤화 효과. 둘쨰, 마감날에 몰리는 현상.
...
ggsave("./pics/Quiz240930_submission_4.png", dpi = 120, width = 960, height = 840, units = "px")
```

---
title: "Quiz 240524 (Birthday Problem, ... , Monty Hall)"
author: "coop711"
date: "2024-05-24"
...
class_roll0517 <- 
  readRDS("./class_roll240517.RDS")
class_roll0517 %<>% 
  data.frame
class_roll <- 
  read_excel("./data/class_roll240524.xlsx", 
             range = "B1:I896")
class_roll %<>% 
  data.frame
str(class_roll)
...
saveRDS(class_roll, file = "./class_roll240524.RDS")
quiz <- "./data/quiz240524.xlsx" %>% 
  read_excel(range = "A1:N703",
...
#> 마감기한으로부터 제출시점의 기간
quiz$days <- 
  as.numeric(difftime("2024-06-07 09:00:00", 
...
랜덤화출석부에 있는 Red, Black 과 실제 구글예습퀴즈에 올린 Red, Black 이 다른 사람들이 있어서 파악해 보았습니다. 랜덤화 효과를 논하기에는 아주 적은 인원이 관찰되었습니다.  Red를 Black 이라고 한 사람이 1명, Black 을 Red 라고 한 사람이 1명 관찰되었습니다. 
...
ggsave("./pics/Quiz240524_Q4_RnB.png", dpi = 120, width = 960, height = 840, units = "px")
```
...
title(main = paste0("Quiz240524 (", N, "명 제출)"))
# title(main = "일 단위 퀴즈 제출 인원", sub = paste0("Quiz231113 (", N, "명 제출)"))
# dev.copy(png, file = "./pics/2401/quiz_submission_13.png", width = 640, height = 480)
invisible(dev.off())
```
...
m2 + geom_text(aes(x = center, y = 1.05), 
  ...
  theme(plot.title = element_text(hjust = 0.5, 
                                  size = 18, 
                                  family = "KoPubWorldDotum Bold")) 
ggsave("./pics/Quiz240524_submission_13.png", dpi = 120, width = 960, height = 840, units = "px")
```
...
library(magick)
file_names <- paste0("./pics/2401/quiz_submission_", 1:13, ".png")
...
```{r, save, echo = FALSE}
saveRDS(quiz, file = "./quiz240524.RDS")
save.image("./quiz240524.RData")
```
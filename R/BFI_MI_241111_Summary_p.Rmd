...
성실성의 경우 "높음"의 백분율은 `r format(BFI_ptbl[1, 3], digits = 1, nsmall = 1)`(%), "낮음"의 백분율은 `r format(BFI_ptbl[4, 3], digits = 1, nsmall = 1)`(%)로 "높음"이 월등히 많습니다. 성실성이 "중하"이하인 사람의 백분율은 `r format(BFI_ptbl[3, 3] + BFI_ptbl[4, 3], digits = 1, nsmall = 1)`(%)로 성실성이 "중상"이상인 사람의 백분율 `r format(BFI_ptbl[1, 3] + BFI_ptbl[2, 3], digits = 1, nsmall = 1)`(%) 보다 적습니다. 성실성의 분포는 마감일이 다가옴에 따라 큰 변화를 보입니다. 특히 "낮음"이 얼마나 늘어나는지 관찰해 보세요. 
...
마감일이 되어서야 질문지에 응답하는 사람들과 그 이전에 응답하는 사람들이 BFP 와 어떤 관련이 있는지 알아 보겠습니다. 다른 성격 특성과 제출 시간과는 두렷한 관계가 나타나지 않지만 성실성은 "낮음"의 경우 마감날에 제출하는 비중이 40%에 가까울 정도로 높고, "중하"에서 "중상"의 경우는 약 30% 내외로 "낮음"에서 "높음" 순으로 갈수록 비중이 줄어드는 것을 알 수 있습니다. 성실성 "높음"의 경우는 미김닐에 제출하는 사람들의 비중이 20%가 채 안됩니다.  중상 이상인 사람들이 마감일 이전에 제출하는 비중이 높다는 것을 알 수 있습니다. 
...
3대 강점지능의 분포를 살펴 보면 인간친화 지능과 자기성찰 지능이 압도적으로 많이 포함되어 있음을 알 수 있습니다. 총 `r BFI_MI_tbl[1, 9]`명이 응답한 가운데 인간친화 지능이 제1순위에 포함된 사람은 `r BFI_MI_tbl[1, 6]`명, 제2순위에 포함된 사람은 `r BFI_MI_tbl[2, 6]`명, 제3순위에 포함된 사람은 `r BFI_MI_tbl[3, 6]`명으로 모두 `r BFI_MI_tbl[1, 6] + BFI_MI_tbl[2, 6] + BFI_MI_tbl[3, 6]`(명)이나 됩니다. 8가지 지능이 3대 강점지능에 골고루 포함된다면 어느 특정 지능이 3대 강점지능에 들어갈 기대인원은 전체인원을 8로 나눈 후 3을 곱해 준`r format(BFI_MI_tbl[1, 9] / 8 * 3, digits = 1, nsmall = 1)`명 정도 기대되는 데 이 보다 훨씬 많은 숫자임을 알 수 있습니다. 그 다음으로 
자기성찰 지능이 제1순위에 포함된 사람은 `r BFI_MI_tbl[1, 7]`명, 제2순위에 포함된 사람은 `r BFI_MI_tbl[2, 7]`명, 제3순위에 포함된 사람은 `r BFI_MI_tbl[3, 7]`명으로 모두 `r BFI_MI_tbl[1, 7] + BFI_MI_tbl[2, 7] + BFI_MI_tbl[3, 7]`(명)이나 됩니다. 그 다음으로는 논리수학 지능이 눈에 띄는 데 논리수학 지능이 제1순위에 포함된 사람은 `r BFI_MI_tbl[1, 3]`명, 제2순위에 포함된 사람은 `r BFI_MI_tbl[2, 3]`명, 제3순위에 포함된 사람은 `r BFI_MI_tbl[3, 3]`명으로 모두 `r BFI_MI_tbl[1, 3] + BFI_MI_tbl[2, 3] + BFI_MI_tbl[3, 3]`(명)입니다. 이제 백분율을 살펴보겠습니다.
...
3대 강점지능의 분포를 백분율로 살펴 보아도 인간친화 지능과 자기성찰 지능이 압도적으로 많이 포함되어 있음을 알 수 있습니다. 인간친화 지능이 제1순위에 포함된 사람의 백분율은 `r format(BFI_MI_ptbl[1, 6], digits = 1, nsmall = 1)`(%), 제2순위에 포함된 사람은 `r format(BFI_MI_ptbl[2, 6], digits = 1, nsmall = 1)`(%), 제3순위에 포함된 사람은 `r format(BFI_MI_ptbl[3, 6], digits = 1, nsmall = 1)`(%) 입니다. 그 다음으로 
자기성찰 지능이 제1순위에 포함된 사람은 `r format(BFI_MI_ptbl[1, 7], digits = 1, nsmall = 1)`(%), 제2순위에 포함된 사람은 `r format(BFI_MI_ptbl[2, 7], digits = 1, nsmall = 1)`(%), 제3순위에 포함된 사람은 `r format(BFI_MI_ptbl[3, 7], digits = 1, nsmall = 1)`(%)입니다. 그 다음으로 논리수학 지능이 제1순위에 포함된 사람은 `r format(BFI_MI_ptbl[1, 3], digits = 1, nsmall = 1)`(%), 제2순위에 포함된 사람은 `r format(BFI_MI_ptbl[2, 3], digits = 1, nsmall = 1)`(%), 제3순위에 포함된 사람은 `r format(BFI_MI_ptbl[3, 3], digits = 1, nsmall = 1)`(%)입니다. 막대그래프로 이 상황을 시각적으로 요약해 보겠습니다.

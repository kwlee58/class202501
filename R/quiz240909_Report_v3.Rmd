...
quiz <- quiz %>%
  mutate(id = replace(id, name == "안현영", "20243414")) %>%
  mutate(id = replace(id, name == "안준선", "20232728")) %>%
  mutate(id = replace(id, name == "김재호", "20242109")) %>%
  mutate(id = replace(id, name == "최윤진", "20243048"))
# quiz[quiz$name == "안현영", "id"] <- "20243414"
...
#   quiz[., c("id", "name")]
quiz[quiz$name %in% c("최재혁", "박지연"), ] #> 최재혁이 중복 제출한 경우
quiz <- quiz %>% 

...
제출시간의 분포가 Red, Black 간에 닮았는지 알아 보았습니다. 분포표 만으로 쉽게 파악할 수 없지만 카이제곱테스트가 도와 줍니다. 카이제곱 통계량은 `r format(days_chisq_test[1], digits = 2, nsmall = 2)`, 자유도는 `r format(days_chisq_test[2], digits = 2, nsmall = 2)`, p-value 는 `r format(days_chisq_test[3], digits = 2, nsmall = 2)` 이므로 제출 시간의 분포는 Red, Black 간에 통계적으로 유의한 차이가 관찰되지 않습니다.
...
제출시간의 분포를 막대그래프로 그려 보았습니다. 마감일에 몰리는 경향이 관찰되고 있습니다.
...
제출시간의 분포를 Red, Black 간에 구분하여 Mosaic Plot 으로 그려 보았습니다. 닮은 것이 느껴지나요?

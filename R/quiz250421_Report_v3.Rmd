...
Red 와 Black 은 선택지의 순서만 바꾸었는데 앞에 놓은 선택지를 고르는 1번효과가 나타나서 통계적으로 유의한 차이를 보여주고 있습니다. (아무런 영향을 주지 않고 있습니다.) 

여기서 수행하고 있는 카이제곱 테스트는 이 순서를 바꾼 것의 효과가 통계적으로 유의한지 알아보기 위한 것입니다. 

그 결과 카이제곱 통계량은 `r chisq_test_q4[1] %>% format(digits = 2, nsmall = 2)`, 자유도는 `r chisq_test_q4[2] %>% format(digits = 2, nsmall = 2)`, p-value 는 `r chisq_test_q4[3] %>% format(digits = 2, nsmall = 2)`이므로 선택지의 순서를 바꾼 것이 앞에 배치한 선택지를 고르는 데 있어서 통계적으로 유의한 차이를 보이고(지) 있(않)습니다. 
...
이를 백분률로 살펴보면 10센트를 먼저 선택지에 올린 Red 에서 정답 5센트를 고른 백분율, `r format(ptbl_q4_a[1, 2], digits = 2, nsmall = 1)`(%)(이)나 5센트를 먼저 선택지에 올린 Black 에서 정답 5센트를 고른 백분율, `r format(ptbl_q4_a[2, 2], digits = 2, nsmall = 1)`(%) 간에 상당히 큰 차이가 있어서 선택지를 바꾸는 것이 응답에 아무런 영향을 주고 있다는 것을 명확히 알 수 있습니다. 

Red, Black 간에 통계적으로 유의한 차이가 관찰되었지만 합쳐서 분석한 결과는 다음과 같습니다.

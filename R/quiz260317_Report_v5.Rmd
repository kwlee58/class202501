...
class_roll0322 <- 
  readRDS("./class_roll_260322.RDS")
class_roll0322 <- data.frame(class_roll0322)
## 수강 철회 반영 새 출석부
# class_roll0928 <- read_excel("./data/class_roll_250928.xlsx", 
#                         range = "B1:H609")
# names(class_roll0928) <- c("dept", "college", "id", "name", "status", "email", "cell_no")
# str(class_roll0928)
# class_roll0928$group <- 
#   class_roll0921$group[match(class_roll0928$id, class_roll0921$id)]
# class_roll_250928 저장
# saveRDS(class_roll0928, file = "./class_roll_250928.RDS")
class_roll <- class_roll0322
sheet_url <- "https://docs.google.com/spreadsheets/d/1NFV_2exoUXwZeeD3j_w4J-NHRlAJca0wKKXlktXGhig"
quiz <- 
  read_sheet(sheet_url)
# quiz <- 
#   read_excel("./data/quiz_250915.xlsx",
#              range = "A2:N290",
#              col_names = FALSE)
names(quiz) <- 
...
as.Date(quiz$time) %>%
  table
# 03-16 제출자 인덱스 찾기
idx <- which(as.Date(quiz$time) == as.Date("2026-03-16"))
...
quiz[quiz$id == "20258044", "time"] <- c("2026-03-17 00:00:01")
str(quiz)
as.Date(quiz$time) %>%
  table
#> 마감기한으로부터 제출시점의 기간
...
#                       units = 'days'))
quiz$days <- 
  as.numeric(difftime("2026-03-31 00:00:00", quiz$time, units = 'days'))
quiz$hours <- 
  as.numeric(difftime("2026-03-31 00:00:00", quiz$time, units = 'hours'))
str(quiz)
# pander(summary(quiz))
```
...
quiz_wrong_ids <- quiz_wrong_ids %>%
  mutate(id = case_when(
    id == "202567145" ~ "20256715",
    id == "20246635" ~ "20246636",
    id == "1" & name == "ㅈ" & dept == "ㅈ" ~ "20255152",
    TRUE ~ id
  ))
...
```{r, cheating charts, echo = FALSE, warning = FALSE}
tbl_q7_a <- 
  quiz$Q7 %>%
  table(quiz$group, ., useNA = "ifany") %>%
  cbind("계" = rowSums(.)) %>%
...
카이제곱 통계량은 `r chisq_test_q7[1] %>% format(digits = 2, nsmall = 3)`, 자유도는 `r chisq_test_q7[2] %>% format(digits = 2, nsmall = 2)`, p-value 는 `r chisq_test_q7[3] %>% format(digits = 2, nsmall = 4)`으로 감세정책의 어떤 측면을 설명하느냐에 따라 반응이 다르게 나온다는 것을 보여줍니다.
...
감세정책의 긍정적 측면을 설명한 Red에서 우리나라의 국민부담률을 "낮춰야 한다"고 응답하는사람들의 백분율, `r format(ptbl_q7_a[1, 1], digits = 2, nsmall = 1)`(%)은 "높여야 한다"고 응답하는 사람들의 백분율, `r format(ptbl_q7_a[1, 2], digits = 2, nsmall = 1)`(%) 보다 조금 많습니다.

감세정책의 부정적 측면을 설명한 Black에서 우리나라의 국민 부담률을 "낮춰야 한다"고 응답하는 사람들의 백분율, `r format(ptbl_q7_a[2, 1], digits = 2, nsmall = 1)`(%)은 "높여야 한다"고 응답하는 사람들의 백분율, `r format(ptbl_q7_a[2, 2], digits = 2, nsmall = 1)`(%) 보다 훨씬 적습니다.
...
m + geom_text(aes(x = center, y = 1.05), 
  ...
  theme(plot.title = element_text(hjust = 0.5, 
                                  size = 18, 
                                  face = "bold")) 
ggsave("./pics/Quiz260317_Q7_RnB.png", dpi = 120, width = 960, height = 840, units = "px")
```

Mosaic Plot 은 이 집계결과를 시각적으로 잘 보여줍니다.

감세정책의 긍정적 측면을 설명한 Red 에서 우리나라의 국민부담률을 "높여야 한다"고 응답한 백분율은 "낮춰야 한다"고 응답한 백분율보다 다소 높지만, 감세정책의 부정적 측면을 설명한 Black 에서 우리나라의 국민부담률을 "높여야 한다"고 응답한 백분율이 월등히 높은 것을 시각적으로 알 수 있습니다.
...
title(main = paste0("Quiz260317 (", N, "명 제출)"))
# title(main = "일 단위 퀴즈 제출 인원", sub = paste0("Quiz230925 (", N, "명 제출)"))
dev.copy(png, file = "./pics/2601/quiz_submission_3.png", width = 640, height = 480)
invisible(dev.off())
})
```
...
이번에는 분포표의 첫번째와 두번째 행, '계'열을 제외한 나머지 열에 대해서 카이제곱테스트를 수행합니다. 카이제곱 통계량은 `r format(days_RnB_chisq_test[1], digits = 2, nsmall = 3)`, 자유도는 `r format(days_RnB_chisq_test[2], digits = 2, nsmall = 2)`, p-value 는 `r format(days_RnB_chisq_test[3], digits = 2, nsmall = 4)` 이므로 제출 시간의 분포는 Red, Black 간에 통계적으로 유의한 차이가 관찰되지 않습니다.
...
ggsave("./pics/Quiz260317_submission_3.png", dpi = 120, width = 960, height = 840, units = "px")
```

```{r, save, echo = FALSE}
saveRDS(quiz, file = "./quiz260317.RDS")
```

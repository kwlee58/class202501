...
class_roll0928 <- readRDS("./class_roll_250928.RDS")
class_roll0928 <- data.frame(class_roll0928)
## 수강 철회 반영 새 출석부
...
sheet_url <- "https://docs.google.com/spreadsheets/d/1tnjSb2AbpvJXIEdU0tMgETMc53n2giA3uffq9ToDKL8"
quiz <- 
...
quiz <- data.frame(quiz)
#> 마감기한으로부터 제출시점의 기간
quiz$days <- as.numeric(difftime("2025-10-06 00:00:00", quiz$time, units = 'days'))
quiz$hours <- as.numeric(difftime("2025-10-06 00:00:00", quiz$time, units = 'hours'))
str(quiz)
pander(summary(quiz))
```
...
ggsave("./pics/Quiz250922_Q7_RnB.png", dpi = 120, width = 960, height = 840, units = "px")
```
...
dev.copy(png, file = "./pics/2501/quiz_submission_4.png", width = 640, height = 480)
invisible(dev.off())
})
```
...
ggsave("./pics/Quiz250922_submission_4.png", dpi = 120, width = 960, height = 840, units = "px")
```

```{r, save, echo = FALSE}
saveRDS(quiz, file = "./quiz250922.RDS")
```
...
abb_MI <- LETTERS[1:8]
M <- abb_MI %>%
  sapply(. %>% {MI[3:58][, `==`(MI[3:58] %>% names %>% substr(1, 1), .)] %>% rowSums})
# M <- 
#   sapply(abb_MI, function(x) rowSums(MI[3:58][, substr(names(MI[3:58]), 1, 1) == x]))
MI_score_kr <- M
MI_names_kr <- 
  c("음악", "신체운동", "논리수학", "공간", "언어", "인간친화", "자기성찰", "자연친화")
dimnames(MI_score_kr) <- list(ID = MI$ID, MI_names_kr)
MI_score_kr %>%
  head %>%
  kable
MI_score_kr %>%
  cbind(MI[paste(LETTERS[1:8], "Score", sep = "_")]) %>%
  head %>%
  kable
```
...
```{r, one data frame}
BFI_MI_full <- 
  data.frame(BFI_full_kr, MI_sort_full_kr[c(2:4)])
BFI_MI_full %>%
  head %>%
  kable(align = "c")
```
...
```{r, BFI absence, echo = FALSE, results = 'markup', eval = FALSE}
class_roll$ID %>%
  setdiff(BFI$ID) 
...
MI결석생은 간단히 전체 출석부의 학번 집합에서 MI 응답자의 학번 집합을 뺸 것임.
시행 당일 외에 늦은 응답도 출석으로 인정하지만, 한 가지 조사만 수행한 경우는 별도 처리.

```{r, MI absence, echo = FALSE, results = 'markup', eval = FALSE}
class_roll$ID %>%
  setdiff(MI$ID)
...
```{r, save, echo = FALSE}
save.image("BFI_MI_report_241111.RData")
```
...
식생활습관 현황을 묻는 질문 후에 식생활 개선 관련 동영상을 시청하면 앞으로 식생활습관을 바꾸겠다고 생각할지 묻는 실험입니다. 즉, 동영상 시청의 효과를 파악하는 실험인데 동일한 대상에게 시청 전과 시청 후의 인식 차이를 물어 보는 것입니다.
...
수강생 각자 동영상 신청 식생활습과 현황조사로부터 점수를 얻었고, 시청 후에 식생활습관 계획조사로부터 습관을 개선했을 때의 점수를 계산하였기 떄문에 그 차이를 개선점수라고 할 수 있습니다. 만약 개선점수가 마이너스이면 동영상 시청의 효과가 없거나 오히려 부정적이라는 뜻이죠. 여기 최소값으로 나와 있는 `r format(summary(food$diff)[1], digits = 2, nsmall = 2)`(은)는 그 경우를 의미합니다. 차이가 플러스일 수록 동영상 시청의 효과를 입증하는 것이죠. 그 차이가 어느 정도 인지를 살펴보기 위하여 먼저 기초통계를 계산한 결과를 위에 제시하였습니다. 여기서 개별 개선점수의  산술평균은 `r format(summary(food$diff)[4], digits = 2, nsmall = 2)`로 상당히 큰 값입니다. 개별 개선점수의 중위수는 `r format(summary(food$diff)[3], digits = 2, nsmall = 2)`로 상당히 많이 개선된 것으로 볼 수 있습니다. 얼마나 개선되었는지를 파악하려면 표준편차를 알아야 하고 이를 자유도의 제곱근으로 나눠 준 표준오차로 나눠 준 t- 통계량이 나오게 됩니다.
...
개선점수의 표준편차는 `r format(sd(food$diff, na.rm = TRUE), digits = 2, nsmall = 2)`입니다. 따라서 개선점수의 산술평균은 차이가 없다고 가정할 때 보다 약 `r format(summary(food$diff)[4]/sd(food$diff, na.rm = TRUE), digits = 2, nsmall = 2)` 배수만큼 큰 개선효과를 의미합니다.
...
동영상 시청의 효과가 없다면 개선점수가 0이거나 평균적으로 마이너스가 나올 것입니다. 이를 통계적으로 검증하기 위한 t-통계값은 `r format(t.test(diff, alternative = "greater")[1], digits = 2, nsmall = 2)`이고 자유도는 시청 전후의 식생활습권 점수가 모두 나와 있는 `r format(grade_tbl_am[1, 5] - summary(food$diff)[7], digits = 2, nsmall = 0)`(명)에서 한 명을 뺸 `r format(t.test(diff, alternative = "greater")[2], digits = 2, nsmall = 0)`이어서 사실상 정규분포라고 보아도 무방합니다. 따라서 p-value 는 `r format(t.test(diff, alternative = "greater")[3], digits = 2, nsmall = 2)`(이)라는 엄청나게 작은 값이 나오고 따라서 동영상시청의 효과는 통계적으로 매우, 매우 유의한 것으로 결론내릴 수 있습니다.
...
개선점수의 분포를 히스토그램으로 나타내었습니다. 그리고 붉은색 선은 개선점수의 산술평균을 평균, 표준편차를 개선점수의 표준편차로 갖는 정규분포의 확률밀도함수입니다. 자유도 `r format(t.test(diff, alternative = "greater")[2], digits = 2)`인 t-분포는 사실상 정규분포와 같기 때문입니다.
...
개선점수의 Box Plot 입니다. 중위수가 `r format(summary(food$diff)[3], digits = 2, nsmall = 2)`인데, 20보다 작다는 것과 제1사분위수가 0과 20의 중간쁨으로 실제는 `r format(summary(food$diff)[2], digits = 2, nsmall = 2)`, 제3사분위수가 20을 좀 넘는 `r format(summary(food$diff)[5], digits = 2, nsmall = 2)`임을 대략 알 수 있습니다. 물론, 최소값이 0보다 작고 최대값은 40을 넘는다는 것도 시각적으로 파악 가능합니다.

## Red and Black (랜덤화 효과)

비록 Red, Black 을 직접 적어 넣으라고 하지는 않았어도 기존에 부여받은 그룹이 있기 때문에 랜덤화 효과를 파악할 수 있습니다. Red, Black 별로 기초통계값을 비교하고 t-test, 그룹별 히스토그램과 Box Plot 을 그려서 시각적으로 비교합니다. 
...
Red 그룹의 개선점수 평균은 `r format(tapply(food$diff, food$group, summary)$Red[4], digits = 2, nsmall = 2)`, Black 그룹의 개선점수 평균은 `r format(tapply(food$diff, food$group, summary)$Black[4], digits = 2, nsmall = 2)` 로 크게 차이가 나지 않습니다. Red 그룹의 개선점수 중위수는 `r format(tapply(food$diff, food$group, summary)$Red[3], digits = 2, nsmall = 2)`, Black 그룹의 개선점수 중위수는 `r format(tapply(food$diff, food$group, summary)$Black[3], digits = 2, nsmall = 2)` 로 크게 차이가 나지 않습니다. 다른 기초통계값들은 각자 비교해 보세요.
...
Red 그룹 개선점수 모집단의 표준편차와 Black 그룹 개선점수 모집단의 표준편차를 알 수 없기 때문에 Welch 의 근사식을 적용한 t-test (Two Sample) (을)를 수행하였습니다. 근사식으로 계산한 자유도는 `r format(t.test(diff ~ group, data = food)[2], digits = 2, nsmall = 2)`, t-통계값은 `r format(t.test(diff ~ group, data = food)[1], digits = 2, nsmall = 4)`이고 따라서 p-value 는 `r format(t.test(diff ~ group, data = food)[3], digits = 2, nsmall = 4)`이 나오므로 Red, Black 개선점수 간의 차이는 통계적으로 유의하지 않고 닮았다고 결론내릴 수 있습니다. 이 비교는 서로 다른지를 묻고 있기 때문에 `r t.test(diff ~ group, data = food)$alternative`, 즉 양측 검증이라는 것을 알 수 있고 Red 그룹 개선점수의 평균, `r format(t.test(diff ~ group, data = food)$estimate[1], digits = 2, nsmall = 2)`(와)과 Black 그룹 개선점수의 평균,  `r format(t.test(diff ~ group, data = food)$estimate[2], digits = 2, nsmall = 2)` 도 요약표에서 확인할 수 있습니다.
...
Red 그룹 개선점수의 히스토그램과 Black 그룹 개선점수의 히스토그램을 아래 위로 그려서 비교하였습니다. 최소값, 제1사분위수, 중위수, 제3사분위수, 최대값이 상당히 닮은 것을 시각적으로 느낄 수 있습니다.
...
Red 그룹 개선점수의 Boxplot과 Black 그룹 개선점수의 Boxplot 을 화면 안에 나란히 배치하여 비교하였습니다. 이 또한 상당히 닮은 것을 시각적으로 느낄 수 있습니다.

### 개별 식생활습관 등급 변화

식생활습관 현황과 계획 조사에 참여하거나 불참한 모든 수강생의 점수입니다. 시청전 점수와 시청후 점수가 어떻게 바뀌고 시청전등급이 시청후등급으로 어떻게 바뀌는 지 살펴보기 바랍니다. 동영상을 시청하고도 개선점수가 마이너스인 사람들은 전체 응답자 `r sum(!is.na(food$diff))`(명) 중에서 총 `r sum(food$diff < 0, na.rm = TRUE)`(명)입니다.

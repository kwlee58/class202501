...
class_roll <- read_excel("./data/class_roll_250905.xlsx", 
                        range = "B1:H617")
str(class_roll)
...
calc_id_chi_square <- function(data) {
  data$id_2 <- data$id %>% substr(1, 4) %>% ifelse(as.numeric(.) <= 2019, "19", .)
...
```{r, echo = FALSE}
hist(Xsum, prob = TRUE, nclass = 50, xlim = c(0, 80), ylim = c(0, 0.06))
x <- seq(0, 80, by = 0.1)

...
class_roll <- readRDS("./class_roll_250222.RDS")
str(class_roll)
```
...
red_and_black <- function(k) {
  ...
  X1 <- calc_id_chi_square(df)
  X2 <- calc_email_chi_square(df)
  X3 <- calc_phone_chi_square(df)
  X4 <- calc_lastname_chi_square(df)
  X5 <- calc_college_chi_square(df)
...
results_all <- list()
batch_size <- 10000
batches <- seq(M1, M2, by = batch_size)

for (start in batches) {
  end <- min(start + batch_size - 1, M2)
  # 병렬처리로 배치 결과 얻기
  batch_results <- mclapply(start:end, red_and_black, mc.cores = num_cores)
  # 기존 results_all 에 새로운 배치 결과 추가
  results_all <- c(results_all, batch_results)
  # 메모리 정리
  gc()  
}
...
# Extract Xsum from the results
Xsum <- unlist(results_all)
# Xsum <- sapply(results_all, function(res) res$Xsum)
# Xsum_list <- sapply(M1:M2, red_and_black)
# Xsum <- Xsum_list$Xsum
names(Xsum) <- M1:M2
Xmin <- names(Xsum[which(Xsum == min(Xsum))])

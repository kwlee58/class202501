...
class_roll$group <- class_roll0901$group[match(class_roll$id, class_roll0901$id)]
na_gr <- which(is.na(class_roll$group)) 
class_roll[na_gr, "group"] <- c("Red", "Black")
saveRDS(class_roll, file = "./class_roll_230916.RDS")
quiz <- 
  read_excel("./data/quiz230904.xlsx",
             range = "A2:N854",
             col_names = FALSE)
...
absentee$group %>%
  table
#> 마감기한으로부터 제출시점의 기간
quiz$days <- as.numeric(difftime("2023-09-18 09:00:00", quiz$time, units = 'days'))
quiz$hours <- as.numeric(difftime("2023-09-18 09:00:00", quiz$time, units = 'hours'))
str(quiz)
```
...
랜덤화출석부에 있는 Red, Black 과 실제 구글예습퀴즈에 올린 Red, Black 이 여전히 다른 사람들이 있어서 파악해 보았습니다. Red를 Black 이라고 한 사람의 수효(27명)와 Black을 Red 라고 한 사람의 수효(17명)에 차이가 많이 나는 것처럼 보이지만 44명이 동전을 던져서 앞면, 뒷면을 나눠보면 44 $\pm\sqrt44/2$ = 22 $\pm$ 3.3 명으로 범위를 살짝 벗어나는 수준입니다. p-value 로 이야기해 보면 동전의 앞면이 17명 이하거나 27명 이상일 확률은 `r round(pbinom(17, size = 44, prob = 0.5) + 1 - pbinom(26, size = 44, prob = 0.5), digits = 3)` 으로 0.05보다 훨씬 큰 값입니다. 
...
include_graphics("./pics/Quiz230904_Q7_Red.png")
```

### Black

```{r, echo = FALSE, out.width = "50%", align = "left"}
include_graphics("./pics/Quiz230904_Q7_Black.png")
```
...
df$Var2 %<>% factor(labels = c("원형그래프", "막대그래프"))
...
ggsave("./pics/Quiz230904_Q7_RnB.png", dpi = 120, width = 960, height = 840, units = "px")
```
...
title(main = paste0("Quiz230904 (", N, "명 제출)"))
# title(main = "일 단위 퀴즈 제출 인원", sub = paste0("Quiz230904 (", N, "명 제출)"))
# dev.copy(png, file = "./pics/2302/quiz_submission_2.png", width = 640, height = 480)
invisible(dev.off())
```
...
```{r, save, echo = FALSE}
saveRDS(quiz, file = "./quiz230904.RDS")
save.image("./quiz230904.RData")
```
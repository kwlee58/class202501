...
library(readxl)
options(width = 180)
class_roll <- read_excel("./data/class_roll240510.xlsx",
                         range = "E1:F897",
                         col_names = TRUE,
                         col_types = "text")
class_roll %<>%
...
```{r}
BFI <- "./data/BFI240510.xlsx" %>% 
  read_excel(range = "A1:AA722",
             col_names = TRUE)
BFI %<>%
  data.frame
BFI %<>%
...
#> 마감일로부터 제출시점의 기간
BFI_days <- as.numeric(difftime("2024-05-24 09:00:00", BFI$Time, 
                                 tz = "Asia/Seoul", units = 'days'))
BFI_hours <- as.numeric(difftime("2024-05-24 09:00:00", BFI$Time, 
...
```{r}
MI <- "./data/MI240510.xlsx" %>% 
  read_excel(range = "A1:BQ700",
             col_names = TRUE)
MI %<>%
  data.frame
MI <- MI[, -2]
...
#> 마감일로부터 제출시점의 기간
MI_days <- 
  as.numeric(difftime("2024-05-24 09:00:00", MI$Time, 
                      tz = "Asia/Seoul", units = 'days'))
MI_hours <- 
  as.numeric(difftime("2024-05-24 09:00:00", MI$Time, 
                      tz = "Asia/Seoul", units = 'hours'))
...
BFI_days %>%
  ...
  rev %>%
  barplot(space = 0, 
          names.arg = rep(c("금", "토", "일", "월", "화", "수", "목"), 2), 
...
```{r, save.image}
saveRDS(BFI, file = "./BFI240510.RDS")
saveRDS(MI, file = "./MI240510.RDS")
save.image("./BFI_MI_240510.RData")
```

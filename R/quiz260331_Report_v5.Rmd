---
title: "Quiz 260331 (Framing Effects)"
author: "coop711"
date: "2026-03-31"
...
#> 출석부에 변화가 생기면 조치
class_roll0413 <- 
  read_excel("./data/class_roll_260413.xlsx", range = "B1:H608")
str(class_roll0413)
names(class_roll0413) <- 
  c("dept", "college", "id", "name", "status", "email", "cell_no")
class_roll0413$group <- 
  class_roll0322$group[match(class_roll0413$id, class_roll0322$id)]
saveRDS(class_roll0413, file = "./class_roll_260413.RDS")
class_roll <- class_roll0413
str(class_roll)
#> googlesheets
sheet_url <- "https://docs.google.com/spreadsheets/d/1f_BcgB-zL31d4ehB9JLX0_4xgKSCNQ6SYFBIe_zx71k"
quiz <- 
  read_sheet(sheet_url)
# quiz <- 
#   read_excel("./data/quiz_250929_v2.xlsx",
#              range = "A2:N560",
#              col_names = FALSE)
...
# 수동 조정할 ID 목록
# target_ids <- c("20266147", "20254110", "20213539", "20241604", "20245202", "20255200", "20258044")

# 시작 시간 설정 및 순차적 시간 생성 (1초씩 증가)
# start_time <- as.POSIXct("2026-03-31 00:00:01")
# new_times <- start_time + (0:(length(target_ids) - 1))

# 한 번에 업데이트
# quiz[quiz$id %in% target_ids, "time"] <- format(new_times, "%Y-%m-%d %H:%M:%S")

# clausee AI 의 제안

ids <- c("20266147", "20254110", "20213539", "20241604", "20245202", "20255200", "20258044")

idx <- match(ids, quiz$id)
quiz$time[idx] <- sprintf("2026-03-31 00:00:%02d", seq_along(ids))

# quiz[quiz$id == "20266147", "time"] <- c("2026-03-31 00:00:01")
# quiz[quiz$id == "20254110", "time"] <- c("2026-03-31 00:00:02")
# quiz[quiz$id == "20213539", "time"] <- c("2026-03-31 00:00:03")
# quiz[quiz$id == "20241604", "time"] <- c("2026-03-31 00:00:04")
# quiz[quiz$id == "20245202", "time"] <- c("2026-03-31 00:00:05")
# quiz[quiz$id == "20255200", "time"] <- c("2026-03-31 00:00:06")
# quiz[quiz$id == "20258044", "time"] <- c("2026-03-31 00:00:07")
str(quiz)
as.Date(quiz$time) %>%
  table
quiz <- data.frame(quiz)
#> 마감기한으로부터 제출시점의 기간
quiz$days <- as.numeric(difftime("2026-04-14 00:00:00", quiz$time, units = 'days'))
quiz$hours <- as.numeric(difftime("2026-04-14 00:00:00", quiz$time, units = 'hours'))
str(quiz)
```
...
quiz_wrong_ids <- quiz_wrong_ids %>%
  mutate(
    id = case_when(
      id == "20040525" ~ "20233064",
      id == "202525611" ~ "20252561",
      id == "20242476" ~ "20242746",
      id == "20050223" ~ "20256165",
      id == "070503" ~ "20263107",
      id == "20256504" ~ "20256503",
      TRUE ~ id
      ),
    name = case_when(
      name == "이소여" ~ "이소연",
...
# literacy_roll <- readRDS("./literacy_roll_250929.RDS")
# class_roll %>%
...
실제로 Red를 Black 이라고 한 사람수, `r wrong_tbl[1, 2]`명이나 Black 을 Red 라고 한 사람수, `r wrong_tbl[2, 1]`명은 기대인원으로부터 표준오차 범위에 잘  들어갑니다.
...
m + geom_text(aes(x = center, y = 1.05), 
  ...
  theme(plot.title = element_text(hjust = 0.5, 
                                  size = 18, 
                                  face = "bold")) 
ggsave("./pics/Quiz250929_Q7_RnB.png", dpi = 120, width = 960, height = 840, units = "px")
```
...
title(main = paste0("Quiz260331 (", N, "명 제출)"))
# title(main = "일 단위 퀴즈 제출 인원", sub = paste0("Quiz230925 (", N, "명 제출)"))
dev.copy(png, file = "./pics/2601/quiz_submission_5.png", width = 640, height = 480)
invisible(dev.off())
})
```
...
ggsave("./pics/Quiz260331_submission_5.png", dpi = 120, width = 960, height = 840, units = "px")
```

```{r, save, echo = FALSE}
saveRDS(quiz, file = "./quiz260331.RDS")
```
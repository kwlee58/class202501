---
title: "직관의 한계 260526 (생일문제, 매칭문제, 연비환상, 몬티 홀 문제)"
author: "coop711"
date : "2026-05-26"
...
class_roll <- 
  readRDS("./class_roll_260525.RDS")
class_roll %<>% 
  data.frame
# class_roll <- 
#   read_excel("./data/class_roll_251220.xlsx", 
#              range = "B1:H605")
# class_roll %<>% 
#   data.frame
# str(class_roll)
# names(class_roll) <- c("dept", "colleges", "id", "name", "status", "email", "cell_no")
# class_roll$group <- class_roll1026$group[match(class_roll$id, class_roll1026$id)]
# saveRDS(class_roll, file = "./class_roll_251220.RDS")
quiz_url <-"https://docs.google.com/spreadsheets/d/1xUbbVEyesYgwF1KpMdO2JTYHjUEDq04R6RuP222zkdg"
quiz <- read_sheet(quiz_url)
# quiz <- "./data/quiz251124.xlsx" %>% 
...
quiz_wrong_ids <- quiz_wrong_ids %>%
  mutate(
    id = case_when(
      id == "2023245" ~ "20232945",
      id == "20232832" ~ "20232833",
      id == "２０２４６６２０" ~ "20246620",
      id == "20239284" ~ "20232984",
      id == "202464616" ~ "20246416",
      id == "ai로봇융합전공" ~ "20256744",
      id == "20253069" ~ "20253060",
      id == "미디어스쿨" ~ "20262526",
    TRUE ~ id
    ),
    name = case_when(
      id == "20258044" & name == "Zhumabaeva Asema" ~ "ZHUMABAEVA ASEMA",
      id == "20248020" & name == "LIUZHOU" ~ "LIU ZHOU",
      id == "20248012" & name == "BAIYUANYUAN" ~ "BAI YUANYUAN",
      id == "20248016" & name == "등시하" ~ "DENG SHIXIA",
    TRUE ~ name
    )
  )
...
#   quiz[., c("id", "name")]
quiz %>%
  filter(id %in% c("20232910", "20255112", "20252709"))
quiz <- quiz %>% 
...
clean_birth_format <- function(x) {
  ...
  x <- chartr("０１２３４５６７８９／", "0123456789/", x)     # 전각 숫자·슬래시 → 반각 (예: "１０／２１" → "10/21")
...
quiz$Q1 <- clean_birth_format(quiz$Q1)
...
quiz[quiz$id == "20256112", "time"] <- c("2026-05-26 00:00:01")
## 자정 직후 제출한 사람의 시간 수동 조정
quiz[quiz$id == "20256733", "time"] <- c("2026-05-26 00:00:02")
## 집계시점으로부터 제출시간까지의 소요일수
quiz$days <- 
  as.numeric(difftime("2026-06-09 00:00:00", 
...
Red, Black 간에 월별 출생의 분포가 닮았는지 알아보기 위하여 수행한 카이제곱 테스트에서 카이제곱 통계량은 `r format(months_RnB_chisq_test[1], digits = 2, nsmall = 3)`, 자유도는 `r format(months_RnB_chisq_test[2], digits = 2, nsmall = 2)`, p-value 는 `r format(months_RnB_chisq_test[3], digits = 2, nsmall = 4)` 이므로 Red, Black 간에 출생의 분포는 통계적으로 유의한 차이를 보이지 않습니다.
...
랜덤하게 골랐다면 `r q2_tbl[1, 5]`(명)을 4로 나눠 준 `r round(q2_tbl[1, 5] / 4, 1)`(명) 정도 나와야 합니다. 
...
이 차이를 한 눈에 볼 수 있도록 맞춘 갯수별로 관찰 인원(Observed)과 기대 인원(Expected)을 나란히 놓고 막대그래프로 비교해 보겠습니다. 

```{r, O_E_barplot, echo = FALSE, message = FALSE, fig.width = 7, fig.height = 5}
#> Observed vs Expected 테이블(O_E_tbl)을 long 형으로 변환 ('계' 열 제외)
O_E_long <- 
  data.frame(
    맞춘갯수 = factor(rep(c("0개", "1개", "2개", "4개"), times = 2),
                   levels = c("0개", "1개", "2개", "4개")),
    구분 = factor(rep(c("Observed", "Expected"), each = 4),
                levels = c("Observed", "Expected")),
    인원 = c(O_E_tbl["Observed", 1:4], O_E_tbl["Expected", 1:4]))
ggplot(O_E_long, aes(x = 맞춘갯수, y = 인원, fill = 구분)) +
  geom_col(position = position_dodge(width = 0.7), width = 0.6) +
  geom_text(aes(label = round(인원)),
            position = position_dodge(width = 0.7),
            vjust = -0.3, family = "kopub", size = 4) +
  theme_bw(base_family = "kopub") +
  labs(title = "맷칭 문제: 관찰 vs 기대 인원",
       x = "맞춘 갯수", y = "인원(명)") +
  scale_fill_brewer(name = "",
                    labels = c("관찰(Observed)", "기대(Expected)"),
                    type = "qual",
                    palette = "Set2",
                    direction = 1) +
  theme(plot.title = element_text(hjust = 0.5, size = 18, face = "bold"))
ggsave("./pics/quiz260526_Q2_OvsE.png", dpi = 120, width = 960, height = 840, units = "px")
```

막대그래프에서 1개를 맞춘 막대가 기대(Expected)보다 관찰(Observed)이 유난히 높게 솟아 있고, 그 반작용으로 0개와 2개의 관찰 막대가 기대보다 낮은 것이 한 눈에 드러납니다. 
...
M19_2 <- c(62, 84)
MPG <- 
  cbind(M19_2, M20_1, M20_2, M21_1, M21_2, M22_1, M22_2, M23_1, M23_2, M24_1, M24_2, M25_1, M25_2, M26_1)
semesters <- 
  c("19/2", "", "20/2", "21/1", "21/2", "22/1", "22/2", "23/1", "23/2", "24/1", "24/2", "25/1", "25/2", "26/1")
...
m + geom_text(aes(x = center, y = 1.05),
  ...
  theme(plot.title = element_text(hjust = 0.5, 
                                  size = 18, 
                                  face = "bold")) 
ggsave("./pics/quiz260526_Q4_RnB.png", dpi = 120, width = 960, height = 840, units = "px")
```
...
M16_1 <- c(41, 15)
quizS <- 
  cbind(M16_1, M16_2, M17_1, M17_2, M18_2, M18_2, M19_1, M19_2, M20_1, M20_2, M21_1, M21_2, M22_1, M22_2, M23_1, M23_2, M24_1, M24_2, M25_1, M25_2, M26_1)
semesters <- 
  c("16/1", "16/2", "17/1", "17/2", "18/1", "18/2", "19/1", "19/2", "20/1", "20/2", "21/1", "21/2", "22/1", "22/2", "23/1", "23/2", "24/1", "24/2", "25/1", "25/2", "26/1")
colnames(quizS) <- semesters
quizS_tbl <- 
  apply(quizS, MARGIN = 2, proportions) %>% 
  `*`(100) %>% 
  round(1)
m_list <- mosaic_gg(as.table(quizS))
m <- m_list$m
df <- m_list$df
p_df <- m_list$p_df
y_min <- min(p_df$y_breaks)
y_max <- sort(unique(p_df$y_breaks[p_df$y_breaks < 0.99]), decreasing = TRUE)[1]
custom_labels <-
  c("16/1", "", "", "", "18/1", "", "19/1", "", "20/1", "", "21/1", "21/2", "22/1", "22/2", "23/1", "23/2", "24/1", "24/2", "25/1", "25/2", "26/1")
m_list$m +
  geom_text(aes(x = center, y = 1.05),
...
title(main = paste0("quiz260526 (", N, "명 제출)"))
# title(main = "일 단위 퀴즈 제출 인원", sub = paste0("quiz230925 (", N, "명 제출)"))
dev.copy(png, file = "./pics/2601/quiz_submission_13.png", width = 640, height = 480)
invisible(dev.off())
})
```
...
ggsave("./pics/quiz260526_submission_13.png", dpi = 120, width = 960, height = 840, units = "px")
```
...
library(magick)
file_names <- paste0("./pics/2601/quiz_submission_", 2:13, ".png")
...
```{r, save, echo = FALSE}
saveRDS(quiz, file = "./quiz260526.RDS")
```
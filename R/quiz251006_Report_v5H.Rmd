---
title: "Quiz 251006 (The more, the better?)"
author: "coop711"
date: "2025-10-06"
...
class_roll1005 <- readRDS("./class_roll_251005.RDS")
class_roll1005 %<>% 
  data.frame
# class_roll0407 <- 
...
class_roll <- class_roll1005
str(class_roll)
sheet_url <- "https://docs.google.com/spreadsheets/d/103IDQo8Cm-UVRHuoRHZ6YaqTqAZ0zFJFERGLVlY7ZiY"
quiz <- 
...
#> 제출시간 분포 분석 코드 
quiz$days <- as.numeric(difftime("2025-10-20 00:00:00", quiz$time, units = 'days'))
quiz$hours <- as.numeric(difftime("2025-10-20 00:00:00", quiz$time, units = 'hours'))
str(quiz)
```
...
quiz_wrong_ids <- quiz_wrong_ids %>%
  mutate(
    id = case_when(
      id == "2055224" ~ "20255224",
      id == "20237015" ~ "20237016",
      id == "20246425" ~ "20236425",
      id == "2021348" ~ "20213348",
      id == "20553251" ~ "20253251",
      id == "2023108" ~ "20231088",
      id == "2025312" ~ "20253012",
      TRUE ~ id
      ),
...
#   quiz[., c("id", "name")]
quiz %>%
  filter(id %in% c("20232504", "20243348", "20242969"))
...
그 결과 카이제곱 통계량은 `r chisq_test_q1[1] %>% format(digits = 2, nsmall = 2)`, 자유도는 `r chisq_test_q1[2] %>% format(digits = 2, nsmall = 2)` , p-value 는 `r chisq_test_q1[3] %>% format(digits = 2, nsmall = 4)`이므로 Red, Black 간에 통계적으로 유의한 차이를 보이지 않습니다. 
...
그 결과 카이제곱 통계량은 `r chisq_test_q3[1] %>% format(digits = 2, nsmall = 2)`, 자유도는 `r chisq_test_q3[2] %>% format(digits = 2, nsmall = 2)`, p-value 는 `r chisq_test_q3[3] %>% format(digits = 2, nsmall = 4)`이므로 Red, Black 간에 **통계적으로 유의한 차이**를 보이고 있습니다. 매우 드물지만 이 또한 낮은 확률로 나타날 수 있다는 것을 보여 줍니다. 스무 번에 한 번? 
...
그렇다면 Red, Black 의 응답은 Q1 ~ Q6 에서와 같이 랜덤화 효과에 의하여 통계적으로 유의한 차이를 보이지 않을 것입니다. 
...
#              family = "kopub",
  ...
  theme(plot.title = element_text(hjust = 0.5, 
                                  size = 18, 
                                  face = "bold")) 
ggsave("./pics/Quiz251006_Q7_RnB.png", dpi = 120, width = 960, height = 840, units = "px")
```
...
2016년 1학기부터 지금까지 시행한 같은 실험들은 꾸준히 "내가 남보다" 많이 받는 선택을 하고 있음을 보여 줍니다. "내가 남보다" 많이 받는 세상을 선택한 퍼센티지는 64.7%에 77.0%에 이를 정도로 압도적입니다.

```{r, echo = FALSE, fig.width = 12, fig.height = 6, message = FALSE}
W25_2 <- c(tbl_q7_a[3, -3])
W25_1 <- c(417, 142)
W24_2 <- c(518, 215)
W24_1 <- c(585, 177)
W23_2 <- c(646, 193)
...
W16_2 <- c(63, 28)
W16_1 <- c(44, 19)
WW <- cbind(W16_1, W16_2, W17_1, W17_2, W18_2, W19_1, W19_2, W20_1, W20_2, W21_1, W21_2, W22_1, W22_2, W23_1, W23_2, W24_1, W24_2, W25_1, W25_2)
semesters <- c("16/1", "16/2", "17/1", "17/2", "18/2", "19/1", "19/2", "20/1", "20/2", "21/1", "21/2", "22/1", "22/2", "23/1", "23/2", "24/1", "24/2", "25/1", "25/2")
...
custom_labels <-
  c("16/1", "", "", "17/2", "", "", "19/2", "", "20/2", "21/1", "21/2", "22/1", "22/2", "23/1", "23/2", "24/1", "24/2", "25/1", "25/2")
m_list$m +
...
카이제곱 통계량은 `r format(days_chisq_test[1], digits = 2, nsmall = 2)`, 자유도는 `r format(days_chisq_test[2], digits = 2, nsmall = 2)`, p-value 는 `r format(days_chisq_test[3], digits = 4, nsmall = 4)` 이므로 날짜별로 고르게 제출하고 있는지 살펴보기에는 천문학적으로 낮은 P-value 가 관찰되었습니다.  
...
title(main = paste0("Quiz251006 (", N, "명 제출)"))
# title(main = "일 단위 퀴즈 제출 인원", sub = paste0("Quiz230925 (", N, "명 제출)"))
dev.copy(png, file = "./pics/2502/quiz_submission_6.png", width = 640, height = 480)
invisible(dev.off())
})
```
...
카이제곱 통계량은 `r format(days_RnB_chisq_test[1], digits = 4, nsmall = 2)`, 자유도는 `r format(days_RnB_chisq_test[2], digits = 2, nsmall = 2)`, p-value 는 `r format(days_RnB_chisq_test[3], digits = 4, nsmall = 2)` 이므로 제출 시간의 분포는 Red, Black 간에 통계적으로 유의한 차이가 관찰되지 않습니다. 
...
ggsave("./pics/Quiz251006_submission_6.png", dpi = 120, width = 960, height = 840, units = "px")
```

```{r, save, echo = FALSE}
saveRDS(quiz, file = "./quiz251006.RDS")
```

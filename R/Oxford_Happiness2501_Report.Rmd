...
options(width = 180)
class_roll <- readRDS("./class_roll_250414.RDS")
#> googlesheets4 패키지 시험
oxford_url <-"https://docs.google.com/spreadsheets/d/1hrQFWBz4RUJiIZcZ270SIFuCKqvIYBbdShFbKCPwhZU"
# gs4_deauth()
oxford <- read_sheet(oxford_url)
...
# oxford$Time1 <- as.POSIXct(oxford$Time1, tz = "Asia/Seoul")
oxford$Time1 <- format(oxford$Time1, format = "%Y-%m-%d %H:%M:%S")
as.Date(oxford$Time1) %>%
  table
str(oxford)
#> 마감기한으로부터 제출시점의 기간
oxford$days <- as.numeric(difftime("2025-05-05 00:00:00", oxford$Time1, tz = "Asia/Seoul", units = 'days'))
oxford$hours <- as.numeric(difftime("2025-05-05 00:00:00", oxford$Time1, tz = "Asia/Seoul", units = 'hours'))
```

```{r, cleaning, echo = FALSE, results = "hide"}
#> 학번 잘못 써서 고생시키는 녀석들 찾아내는 코드
# 1. 잘못된 id 또는 name 가진 행 찾기
oxford_wrong_ids <- oxford %>%
  anti_join(class_roll, by = c("id", "name")) 

# 2. 수정 전 원래의 id와 name 쌍을 저장
wrong_keys <- oxford_wrong_ids %>% 
  select(id, name)

# 3. 잘못된 id 또는 name 수정
oxford_wrong_ids <- oxford_wrong_ids %>%
  mutate(
    id = case_when(
      id == "202225259" ~ "20225259",
      id == "20221551" ~ "20221541",
      id == "200243608" ~ "20243608",
      id == "20236381" ~ "20236281",
      id == "021024" ~ "20211713",
      id == "02246605" ~ "20246605",
      id == "20243521" ~ "20243532",
    TRUE ~ id
    ),
    name = case_when(
      name == "이은ㅇ여" ~ "이은영",
...
# 3. oxford 에서 id 또는 name 틀린 행을 제거하고 옳은 id 또는 name 갖춘 행으로 교체하기
oxford <- oxford %>%
  anti_join(wrong_keys %>% select(id, name), by = c("id", "name")) %>%
  bind_rows(oxford_wrong_ids)
...
#   oxford[., c("id", "name")]
oxford %>%
  filter(id %in% c("20243211"))
# oxford[oxford$id %in% c("20243211"), ] #> 20243211이 중복 제출
oxford <- oxford %>% 
...
#> RDS 포맷으로 저장 
saveRDS(oxford, file = "./oxford_250421.RDS")
```
...
즉, 통계적으로 유의한 차이를 관찰하(지 못하)였습니다. 

국문 행복도 조사와 영문 행복도 조사의 평균 행복점수 차이는 랜덤화의 효과로 충분히 나올 수 있는 수준이라는 것입니다. 

결과 요약표에는 각 집단의 평균 행복점수도 나와 있습니다. 

지난 학기의 자료들을 살펴보면 국문으로 물어 봤을 때하고 영문으로 물어 봤을 행복 점수에서 통계적으로 유의한 차이가 있었다는 것을 알 수 있습니다. 
...
총 `r oxford_tbl[3, 6]` 명 응답한 가운데 `r sum(oxford_tbl[3, 4:5])`명이 꽤 행복하다 이상으로 행복하다고 응답하였고 그저 그렇다고 응답 한 사람들이 `r oxford_tbl[3, 3]` 명이었습니다. 
...
그 결과 카이제곱 통계량은 `r format(oxford_chisq_test[1], digits = 2, nsmall = 2)`, (반복수 2,0000의 붓스트랩 근사를 사용하였기 때문에) 자유도는 `r format(oxford_chisq_test[2], digits = 2, nsmall = 2)`, p-value 는 `r format(oxford_chisq_test[3], digits = 2, nsmall = 4)`이므로 통계적으로 유의한 차이를 보이지(고) 않(있)습니다.
...
국문, 영문 행복도 조사의 소요 시간에 대해서도 t-test 를 수행하였는데 t-통계량은 `r format(Duration_t_test[1], digits = 2, nsmall = 2)`, 자유도는 `r format(Duration_t_test[2], digits = 2, nsmall = 2)`, p-value 는 `r format(Duration_t_test[3], digits = 2, nsmall = 2)`이어서 통계적으로 유의한 차이가 관찰되(지 않)고 있습니다.
...
title(main = paste0("oxford250421 (", N, "명 제출)"))
# title(main = "일 단위 퀴즈 제출 인원", sub = paste0("oxford230925 (", N, "명 제출)"))
dev.copy(png, file = "./pics/2501/oxford_submission_8.png", width = 640, height = 480)
invisible(dev.off())
})
```

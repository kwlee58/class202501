...
##   시행 횟수가 헤더 주석 · PC 블록 · Mac 블록 세 곳에 흩어져 있어 어긋나기 쉬웠다.
...
VARS_LABEL <- "5 Variables"            # 문서 제목에 쓸 이름
```
...
## 2026-09-04 수정 : range = "B1:H618" 지정을 걷어냈다.
## 출석부 구조(A열 순번, B열부터 자료, 맨 끝 교수자·조교)는 그대로지만
## 인원이 학기마다 달라 마지막 행 번호를 매번 고쳐야 했다.
## 머리글 이름으로 열을 찾고, 못 찾으면 예전처럼 B~H열로 물러선다.
...
## knit 중 RStudio 콘솔에 찍힌다. 탐색을 시작하기 전에 이것부터 확인할 것.
message("\n[열 대응 확인]")
...
## 2026-09-04 신설 : 실제로 돈 시행 범위를 문서에 찍는다.
## RPubs Publish 창의 제목에 이 줄을 그대로 옮겨 적으면 제목과 내용이 어긋나지 않는다.
...
pchisq(9, df = 32) %>%
  as.data.frame() %>%
  setNames("Pr(X <= 9)") %>%
  mutate(`Pr(X <= 9)` = formatC(`Pr(X <= 9)`, format = "e", digits = 2)) %>%
  pander()
...
```{r, echo = FALSE}
hist(Xsum, prob = TRUE, nclass = 50, xlim = c(0, 80), ylim = c(0, 0.08))
x <- seq(0, 80, by = 0.1)
...
class_roll %$%
  table(.$group, .$college) %>%
  pander(col.names = c("간호", "경영", "글로벌융합", "미디어", "미래융합", "반도체/디스플레이", "사회과학", "의과", "인문", "자연과학", "정보과학"))
# 데이터 프레임 생성
result5 <- data.frame(
  Statistic = round(X5_min, 2),
  P.value = round(1 - pchisq(X5_min, df = 10), digits = 4)   ## 2026-09-04 수정 : 단과대학 11개 -> df 10
)
# 열 이름 수정 (P.value -> P-value)

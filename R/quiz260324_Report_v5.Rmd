...
class_roll0322 <- readRDS("./class_roll_260322.RDS")
class_roll0322 <- data.frame(class_roll0322)
## 수강 철회 반영 새 출석부
class_roll0406 <- read_excel("./data/class_roll_260406.xlsx", 
                        range = "B1:H608")
names(class_roll0406) <- c("dept", "college", "id", "name", "status", "email", "cell_no")
str(class_roll0406)
class_roll0406$group <- 
  class_roll0322$group[match(class_roll0406$id, class_roll0322$id)]
saveRDS(class_roll0406, file = "./class_roll_260406.RDS")
class_roll <- class_roll0406
sheet_url <- "https://docs.google.com/spreadsheets/d/13UsSp5g_6MXIIVjijm6VB9a4oVBQQ2P7I_rEFfPCoXg"
quiz <- 
  read_sheet(sheet_url)
# quiz <- 
#   read_excel("./data/quiz_250922.xlsx",
#              range = "A2:N150",
#              col_names = FALSE)
...
quiz$Q1 <- 
  factor(quiz$Q1, 
         ...
         labels = c("과전법 체제에서 전국 토지를 세 등급으로 나누고 실제 수확량을 확인하여 징수하였다.", 
                    "‘조(租)’는 공전(公田)의 경작자가 국고에 상납하는 지대 또는 사전(私田)의 경작자가 전주에게 바치는 지대를 뜻한다.", 
                    "‘세(稅)’는 사전의 소유자가 국가에 상납하는 지세를 뜻한다.", 
                    "실제로 현장에 나가서 수확량을 파악하고 등급을 매기는 답험(踏驗)을 하였다.", 
                    "등급에 따라 일정한 비율로 세금을 감면해 주는 대동(大同)법을 실시하였다."))
...
quiz <- data.frame(quiz)
#> 마감기한으로부터 제출시점의 기간
quiz$days <- as.numeric(difftime("2026-04-07 00:00:00", quiz$time, units = 'days'))
quiz$hours <- as.numeric(difftime("2026-04-07 00:00:00", quiz$time, units = 'hours'))
str(quiz)
pander(summary(quiz))
```
...
quiz_wrong_ids <- quiz_wrong_ids %>%
  mutate(
    id = case_when(
      id == "20223539" ~ "20223529",
      id == "252362" ~ "20252362",
      id == "220253511" ~ "20253511",
      id == "20243713" & name == "김지혜" ~ "20242713",
      TRUE ~ id
      ),
    name = case_when(
      name == "Zhumabaeva Asema" ~ "ZHUMABAEVA ASEMA",
      name == "등시하" ~ "DENG SHIXIA",
      name == "LIUZHOU" ~ "LIU ZHOU",
      name == "문서여" ~ "문서영",
      TRUE ~ name
      )
    )
...
#   quiz[., c("id", "name")]
quiz %>%
  filter(id %in% c("20245139", "20256146"))
...
실제로 Red를 Black 이라고 한 사람수, `r wrong_tbl[1, 2]`명이나 Black 을 Red 라고 한 사람수, `r wrong_tbl[2, 1]`명은 기대인원으로부터 표준오차 범위, 혹은 표준오차 두 배 범위에는 잘 들어갑니다.
...
include_graphics("./pics/Quiz_250922_Q1.png")
```
...
그렇다면 Red, Black 의 응답은 대부분의 Q1 ~ Q6 에서와 같이 랜덤화 효과에 의하여 통계적으로 유의한 차이를 보이지 않을 것입니다. 
...
반면 반대를 유도하는 부연설명을 붙인 Black에서 "찬성한다"고 응답하는 사람들의 백분율, `r format(ptbl_q7_a[2, 1], digits = 2, nsmall = 1)`(%)은 "반대한다"고 응답하는 사람들의 백분율, `r format(ptbl_q7_a[2, 2], digits = 2, nsmall = 1)`(%) 보다 적습니다. 
...
"찬성한다"고 응답한 백분율은 Red, Black 합쳐서  `r format(ptbl_q7[1], digits = 2, nsmall = 1)`(%)(으)로  '반대한다"고 응답한 백분율, `r format(ptbl_q7[2], digits = 2, nsmall = 1)`(%) 보다 약간 많습니다. 

그리고, 모름/무응답이 `r format(ptbl_q7[3], digits = 2, nsmall = 1)`(%)로 적지 않습니다.
...
m + geom_text(aes(x = center, y = 1.05), 
  ...
  theme(plot.title = element_text(hjust = 0.5, 
                                  size = 18, 
                                  face = "bold")) 
ggsave("./pics/Quiz260324_Q7_RnB.png", dpi = 120, width = 960, height = 840, units = "px")
```

Mosaic Plot 은 이 집계결과를 시각적으로 잘 보여줍니다. 

찬성을 유도하는 부연설명을 붙인 Red 에서 "찬성한다"고 응답한 백분율이 높고, 반대를 유도하는 부연설명을 붙인 Black 에서 "반대한다"고 응답한 백분율이 높은 것을 시각적으로 알 수 있습니다. 
...
title(main = paste0("Quiz260324 (", N, "명 제출)"))
# title(main = "일 단위 퀴즈 제출 인원", sub = paste0("Quiz230925 (", N, "명 제출)"))
dev.copy(png, file = "./pics/2601/quiz_submission_4.png", width = 640, height = 480)
invisible(dev.off())
})
```
...
ggsave("./pics/Quiz260324_submission_4.png", dpi = 120, width = 960, height = 840, units = "px")
```

```{r, save, echo = FALSE}
saveRDS(quiz, file = "./quiz260324.RDS")
```
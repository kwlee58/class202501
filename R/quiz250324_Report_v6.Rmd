...
class_roll <- readRDS("./class_roll_250321.RDS")
class_roll <- data.frame(class_roll)
sheet_url <- "https://docs.google.com/spreadsheets/d/17Jzwh3Erda7boIIo0uEQd-cWpQ4oHxnn6gWMCDJHgm4"
quiz <- 
  read_sheet(sheet_url)
# quiz <- 
#   read_excel("./data/quiz_250324.xlsx",
#              range = "A2:N150",
#              col_names = FALSE)
...
quiz <- data.frame(quiz)
#> 마감기한으로부터 제출시점의 기간
quiz$days <- as.numeric(difftime("2025-04-07 00:00:00", quiz$time, units = 'days'))
quiz$hours <- as.numeric(difftime("2025-04-07 00:00:00", quiz$time, units = 'hours'))
str(quiz)
```
...
quiz_wrong_ids <- quiz_wrong_ids %>%
  mutate(
    id = case_when(
      id == "202216743" ~ "20226743",
      id == "20224114" ~ "20224116",
      id == "20040813" ~ "20242715",
...
# 4. quiz 에서 id 또는 name 틀린 행을 제거하고 옳은 id 또는 name 갖춘 행으로 교체하기
quiz <- quiz %>%
...
#   quiz[., c("id", "name")]
quiz %>%
  filter(id %in% c("20241629", "20241514"))
...
m + geom_text(aes(x = center, y = 1.05), 
  ...
  theme(plot.title = element_text(hjust = 0.5, 
                                  ...
                                  family = "kopub",
                                  face = "bold")) 
ggsave("./pics/Quiz250324_Q7_RnB.png", dpi = 120, width = 960, height = 840, units = "px")
```
...
title(main = paste0("Quiz250324 (", N, "명 제출)"))
...
ggsave("./pics/Quiz250324_submission_4.png", dpi = 120, width = 960, height = 840, units = "px")
```

```{r, echo = FALSE}
saveRDS(quiz, file = "./quiz250324.RDS")
#save.image("./quiz250324.RData")
```
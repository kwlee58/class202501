...
literacy_roll <- 
  read_excel("./data/literacy_240930.xlsx",
             range = "C1:AD743",
             col_names = TRUE,
             col_types = "text") 
# literacy_roll <- literacy_roll[, -c(1, 3)]
...
kable(literacy)
#> RDS 포맷으로 저장 
saveRDS(literacy_roll, file = "./literacy_roll240930.RDS")
```
...
# png(file = "./pics/Literacy_240930.png", width = 640, height = 840)
...
# dev.copy(png, file = "./pics/Literacy_240930.png", width = 640, height = 840)
invisible(dev.off())
```
...
#  coord_flip()
ggsave(file = "./pics/Literacy_240930_ggplot.png", bg = "white", dpi = 72, width = 840, height = 480, units = "px")
```
...
Red 와 Black으로부터 관찰된 점수들의 평균에 대하여 t test를 적용하였더니 기초통계량에서 살핀 것을 뒷받침해 주듯이 p-value 가 매우 큰 값이 나왔습니다. 평균은 매우 닮았다는 뜻입니다.

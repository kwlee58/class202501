---
title: "Quiz 260512 (Anchor Effects)"
author: "coop711"
date : "2026-05-12"
...
class_roll0518 <-
  readRDS("./class_roll_260518.RDS")
class_roll0518 %<>%
  data.frame
class_roll <-
  read_excel("./data/class_roll_260525.xlsx",
             range = "B1:H603")
class_roll %<>%
  data.frame
str(class_roll)
names(class_roll) <- c("dept", "colleges", "id", "name", "status", "email", "cell_no")
class_roll$group <- class_roll0518$group[match(class_roll$id, class_roll0518$id)]
saveRDS(class_roll, file = "./class_roll_260525.RDS")
#> 
quiz_url <-"https://docs.google.com/spreadsheets/d/1Jb27l1vxMiqZokAU4_h2x-Bhrp7HNaU-wHGY3Qle-2E"
# gs4_deauth()
quiz <- read_sheet(quiz_url)
# quiz <- 
#   read_excel("./data/quiz251110.xlsx",
#              range = "A2:N705",
#              col_names = FALSE)
str(quiz)
names(quiz) <- 
...
as.Date(quiz$time) %>%
  table
#> 마감기한으로부터 제출시점의 기간
quiz$days <- as.numeric(difftime("2026-05-26 00:00:00", quiz$time, 
                                 tz = "Asia/Seoul",  units = 'days'))
quiz$hours <- as.numeric(difftime("2026-05-26 00:00:00", quiz$time, 
...
quiz_wrong_ids <- quiz_wrong_ids %>%
  mutate(
    id = case_when(
      id == "29256745" ~ "20256745",
      id == "20253014" ~ "20253104",
    TRUE ~ id
    ),
    name = case_when(
...
```{r, mosaic plot0, echo = FALSE, fig.width = 7, fig.height = 5}
library(ggplot2)
source("./mosaic_gg.R")
m_list <- quiz$Q7 %>%
  table(quiz$group, ., useNA = "ifany") %>%
...
이를 백분율로 살펴보면 Red 에서 100만원 이하를 적정 수준이라고 답한 백분율, `r format(ptbl_q7_a2[1, 1], digits = 2, nsmall = 1)`(%)은 200만원 이상을 적정 수준이라고 답한 백분율, `r format(ptbl_q7_a2[1, 2], digits = 2, nsmall = 1)`(%) 보다 상당히 높습니다. 반면 Black에서 100만원 이하를 적정 수준이라고 답한 백분율, `r format(ptbl_q7_a2[2, 1], digits = 2, nsmall = 1)`(%)은 200만원 이상을 적정 수준이라고 답한 백분율, `r format(ptbl_q7_a2[2, 2], digits = 2, nsmall = 1)`(%) 보다 월등히 낮습니다. 
...
title(main = paste0("Quiz260512 (", N, "명 제출)"))
# title(main = "일 단위 퀴즈 제출 인원", sub = paste0("Quiz230925 (", N, "명 제출)"))
dev.copy(png, file = "./pics/2601/quiz_submission_11.png", width = 640, height = 480)
invisible(dev.off())
})
```
...
ggsave("./pics/Quiz260512_submission_11.png", dpi = 120, width = 960, height = 840, units = "px")
```

```{r, save, echo = FALSE}
saveRDS(quiz, file = "quiz260512.RDS")
```